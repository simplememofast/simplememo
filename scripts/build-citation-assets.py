...
<script>try{var k='simple-memo-lang';if(!localStorage.getItem(k))localStorage.setItem(k,'en')}catch(e){}</script>""" if english else ""
    ...
    copyright_line = constants["copyrightLineEn"] if english else constants["copyrightLine"]
    html = f"""<!DOCTYPE html>
<html lang="{lang}">
<head>
<meta charset="utf-8">
...
<a href="{prefix}/contact">{'Contact' if english else 'お問い合わせ'}</a>
...
<p>To inspect the product behind these records, see <a href="/en/obsidian/">the Obsidian workflow</a>, <a href="/en/siri/">Siri setup</a>, or <a href="/en/press/">the product facts and media kit</a>. For corrections or reproducibility questions, <a href="/en/contact">contact the developer</a>.</p></section>
"""
...
<h2>公開中の対話メモ</h2><p><a href="https://apps.apple.com/jp/app/id6758438948">現在のApp Store掲載</a>では、メモ入力画面で短い追加質問に答え、内容を確認して既存の送信ボタンを使う機能を案内しています。質問と整理には、iOS 26以降の対応するiPhone、Apple Intelligenceの有効化、対応言語モデルが必要です。音声入力にはマイクの許可も必要です。利用できない場合も通常の文字入力・音声入力は残ります。対話を終えただけでは、保管庫への保存、メールの配送、他端末への同期は確認できません。それぞれ別に確かめてください。</p>
...
<h2>Compare capture workflows fairly</h2><p>The CSV is a blank measurement worksheet, not a table of results. It has 10 trials for each of five example methods: direct Obsidian entry, Apple Shortcuts, Simple Memo, email-to-self, and a share sheet. Record your actual setup; some methods require extra configuration to reach the same destination.</p><p>Use the same device and OS, define the start and end of “ready to capture,” separate warm from cold starts, and record both successful and failed trials. Record the number of actions and time to appear in the destination separately. Do not infer that 50 trials represent all users. See the <a href="/en/blog/benchmark-methodology">published measurement methodology</a> for context.</p>
<h3>Keep evidence for each trial</h3><p>Start with an empty editor and a synthetic note. Record the app build and distribution channel, capture route, network state, and whether setup and sign-in are complete. Film the starting action and the resulting note in the destination continuously. Keep the recording reference private; do not include personal notes or account identifiers in a shared worksheet.</p><p>Use seconds from the same recording timeline for the starting action, input-ready state, send action, and first visible destination result. Leave a timestamp blank when obscured or outside the recording; do not substitute a screenshot clock or a backend event clock. An empty editor or an in-app history entry does not establish delivery. Define destination delay as destination-visible time minus send-action time, only when both are visible. Record a failure or an unknown result and its reason instead of silently dropping the trial. The first 10 rows per method are a starting worksheet, not a statistical sample-size recommendation.</p>
...
<h3>試行ごとの証拠を残す</h3><p>編集欄を空にし、個人情報を含まないテスト文を使います。アプリのbuildと配布経路、入力経路、通信状態、設定・認証の完了状態を記録してください。開始操作から保存先の本文が見えるまでを同じ動画に収めます。録画の参照情報は手元で管理し、共有するシートに私的なメモやアカウント識別子を含めないでください。</p><p>開始操作、入力可能な状態、送信操作、保存先で初めて本文を確認できた時点を、同じ動画の先頭からの秒数で記録します。指で隠れた時点や録画外の時点は空欄にし、スクリーンショットの時計やサーバーイベントの時計で補いません。編集欄の消去やアプリ内履歴だけでは到着を確認できません。保存先への到着時間は、両端が見える場合だけ「保存先の表示時点−送信操作時点」で算出します。失敗・未確認とその理由も残します。各方式10行は記入の出発点であり、統計上必要な試行数の推奨ではありません。</p>
<h2>例文作成から、自動で残す運用へ</h2><p>ショートカットからObsidianのノートを開いたり追記したりするURLが必要なら、<a href="/resources/obsidian-uri/">Obsidian URIジェネレーター</a>を使えます。このMarkdownツールとは別に、保管庫名やノートのパスをURI用にエンコードします。</p><p>iPhoneから保管庫へ自動で追記する場合は、<a href="/obsidian/">シンプルメモのObsidian設定ガイド</a>をご覧ください。フォルダ選択、Inbox・デイリーノートの使い分け、オフライン時、Obsidianが開く代替経路を説明しています。音声入力は<a href="/siri/">Siriの設定</a>へ。フォルダやノート名は自分の運用に合わせて選べます。</p><p>使い方を記事で紹介するときは、読者が例を変更できるこのページを案内できます。製品説明や設定画面は<a href="/press/">紹介用資料</a>にまとめています。</p>"""
...
put("assets/downloads/capture-measurement-worksheet.csv", csv_text(
    ["method", "trial", "device", "os_version", "app_version", "warm_or_cold", "start_definition", "end_definition", "ready_seconds", "actions_count", "destination", "destination_delay_seconds", "success", "notes", "app_build", "distribution_channel", "capture_route", "network_state", "setup_and_auth_state", "recording_reference", "start_video_seconds", "ready_video_seconds", "send_video_seconds", "destination_visible_video_seconds", "outcome_reason"],
    [[method,i]+[""]*23 for method in ["Obsidian direct entry","Apple Shortcuts","Simple Memo","Email to self","Share sheet"] for i in range(1,11)]))
