...
<script>try{var k='simple-memo-lang';if(!localStorage.getItem(k))localStorage.setItem(k,'en')}catch(e){}</script>""" if english else ""
    ...
    if "https://apps.apple.com/" in body:
        scripts += f'\n<script src="{asset("js/app-store-tracking.js")}" defer></script>'
...
<p>For a review, test the currently listed App Store build on your own device. Version, availability, and pricing may change after this 25 September 2026 snapshot.</p>
...
<p>レビュー時は、掲載中のApp Store版をお手元の端末でお確かめください。バージョン、提供状況、料金は、この2026年9月25日の資料作成後に変わる場合があります。</p>
