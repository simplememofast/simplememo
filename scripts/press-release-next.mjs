...
export function render(manifest) {
  ...
  const body = `株式会社ユリカ（東京都渋谷区）は、iPhone／Apple Watch向けアプリ「シンプルメモ」で、書いたり話したりしたメモをNotionへ保存する連携機能の提供を開始しました。既存の自分宛メール、Obsidianへの記録に加え、Notionの「SimpleMemo Inbox」へ1メモ1ページで保存できます。
