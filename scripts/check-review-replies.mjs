 ...
 *   下書きを検査する … 同上                        → AIが実行
 *   **投稿する**     … 公開。不可逆                → **品質ゲートを通れば自動投稿**
 *                                                  （2026-08-27 にオーナーが決めた）
 *
 * **このファイルは投稿しない。**ASC への POST は
 * simplememo-ios/scripts/asc_review_reply.rb にある（鍵があちらにしか無い）。
 * **判断はこちら、実行はあちら。**あちらは `--auto-post-plan` に ASC の実物を渡して
 * 計画をもらい、decision=="post" のものだけ POST する。
 * ゲートの写しはあちらに置いていない —— 写しは必ずずれ、ずれたとき
 * **厳しいほうが動かなくなっても誰も気づかない。**
 *
 * 【何を落とすか】
...
if (isMain) {
  ...
  console.log(`  **このスクリプトは投稿しない。**POST は `
    + `simplememo-ios/scripts/asc_review_reply.rb にある（鍵があちらにしか無い）。`);
  // **ここに出る hold を「投稿されない」と読まない。**この表示は台帳だけで
  // ゲートを通しているので、星も本文も無い。実行側は ASC の実物を渡すので、
  // 「rating が無い」は消えて別の判定になる。
  console.log(`  上の判定は**台帳だけで通した結果。**星と本文と返信の有無は ASC の実物が埋めるので、`
    + `実行側（--auto-post-plan）では別の理由に変わる。`);
  // [2026-08-27] ここに「実行側では止まらない」と書いていた。**実測は逆。**
  // 初回の実運転（run 33028844739）は2件とも hold で、理由は
  // 「ASC 側にすでに返信が付いている」だった。**台帳の posted_at が null でも、
  // ストアには返信が付いている。**推測で「通る/止まる」を書かないこと。
  console.log(`  **どちらに転ぶかはここでは分からない。**実測（2026-08-27）では、`
    + `台帳が「rating が無い」で止めた2件を、実行側は`
    + `「**ASC 側にすでに返信が付いている**」で止めた —— `
    + `**この台帳の posted_at は、ストアの実態と一致しない。**`);
