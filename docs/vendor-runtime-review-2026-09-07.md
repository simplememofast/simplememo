...
再委託先と認証SLAの追加読解は [Firebaseの追加審査](firebase-subprocessor-sla-review-2026-09-07.md) に記録した。対象サービス・操作・最低リクエスト数の条件を区別し、Blazeプランの観測だけでIdentity PlatformへのアップグレードやSLA適用を認定しない。

## 公開範囲

AppsFlyerのDPA・再委託先・SLAとSDK停止範囲の照合は [AppsFlyerの追加審査](appsflyer-runtime-review-2026-09-07.md) に記録した。イベント送信フラグとSDK全体の停止を区別し、一括削除の90日を最長期限として扱わない。
