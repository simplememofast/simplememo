...
| 指標 | 値 | 分母 |
|---|---:|---|
| **総合自動化率** | **39.9%** | 定義タスク 173（未実装を含む・**最も厳しい**） |
| AI実行率 | 56.6% | 実施中タスク 122（未実装を除く） |
| AI関与率 | 83.6% | 同上（提案・下書きまで含める・**最も甘い**） |
| カバー率 | 70.5% | そもそも誰かがやっているタスクの割合 |

内訳: 自律 8 / ゲート付き実行 61 / 提案 33 / 人間 20 / **未実装 51** / 意図的にやらない 2
...
- **AI関与率 83.6%** — 「誰かがやっているタスクの8割にAIが関わっている」。**この数字だけ見ると誤解する**
- **総合自動化率 39.9%** — 「あるべきタスクのうちAIが実行しているのは4割」。**これが現在地**
- **カバー率 70.5%** — 3割は**誰もやっていない**。自動化以前に未着手

つまり **AIが関わっている領域では実際に手を動かしているが、そもそも手が付いていない領域が3割ある**。
...
|---|---:|---:|---:|---:|---|
| ③ 自律型マーケティング | **61.5%** | 76.2% | 90.5% | 80.8% | 3/13/3/2/5 |
| ① 次期機能開発 | **60.0%** | 69.2% | 100.0% | 86.7% | 3/6/4/0/2 |
| ② バグ修正 | **58.8%** | 71.4% | 92.9% | 82.4% | 1/9/3/1/3 |
...
| ⑦ 法人経営 | **0.0%** | 0.0% | 60.0% | 38.5% | 0/0/3/2/8 |
| ⑬ アナログ領域 | **0.0%** | 0.0% | 12.5% | 61.5% | 0/0/1/7/5 |

### この表から読めること

1. **③マーケティングと①次期機能開発が先頭。**マーケは毎朝の記事ループが実際に回っているから。②はカナリアガード・監視棚卸し・独立2モデル監査が2026-08-22〜23に入った分。⑩AgentOpsも同水準（運転台帳・権限表・自己修復・切替演習）。
2. **⑦法人経営は総合0.0%のまま。**13タスク中8が未実装。2026-08-25に経営者が10社の契約条件を実際にレビューし、条項検査が human_only になってカバー率は30.8%→38.5%。**それでもAIが実行に回った項目はゼロ**。「構想中心」という外部レビューの評価は変わっていない。
3. **⑬アナログ領域の総合0.0%は正常。**7タスクが `human_only` で、これは失敗ではなく**設計**。ただし**AIが担える側（イベント発注・人事の日程調整・制度探索・定型契約・営業リード）は全部未実装**なので伸ばす余地は残る。
...
総合 **61.5%** ／ 実行 76.2% ／ 関与 90.5% ／ カバー 80.8%
...
総合 **58.8%** ／ 実行 71.4% ／ 関与 92.9% ／ カバー 82.4%
...
総合 **0.0%** ／ 実行 0.0% ／ 関与 60.0% ／ カバー 38.5%
