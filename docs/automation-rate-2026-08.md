...
| 指標 | 値 | 分母 |
|---|---:|---|
| **総合自動化率** | **34.5%** | 定義タスク 170（未実装を含む・**最も厳しい**） |
| AI実行率 | 51.8% | 実施中タスク 107（未実装を除く） |
| AI関与率 | 83.3% | 同上（提案・下書きまで含める・**最も甘い**） |
| カバー率 | 66.7% | そもそも誰かがやっているタスクの割合 |

内訳: 自律 6 / ゲート付き実行 53 / 提案 36 / 人間 19 / **未実装 57** / 意図的にやらない 2
...
- **AI関与率 83.3%** — 「誰かがやっているタスクの8割にAIが関わっている」。**この数字だけ見ると誤解する**
- **総合自動化率 34.5%** — 「あるべきタスクのうちAIが実行しているのは3割」。**これが現在地**
- **カバー率 66.7%** — 3割強は**誰もやっていない**。自動化以前に未着手
...
| **未実装** | 保持期間の**自動削除** | 23ストア中5つだけ。**11ストアが個人紐づき識別子を無期限保持** |
