...
> **測定日: 2026-08-22 / 172タスク / 13領域**
> 台帳: `data/automation-coverage.json` ／ 集計: `node scripts/automation-rate.mjs`
> タスクの列挙は `autopilot-maturity-backlog.md`（外部レビュー由来）＋本セッションの実測。
> **分類は3リポジトリの実装を1件ずつ当てて決めた。**
...
| 指標 | 値 | 分母 |
|---|---:|---|
| **総合自動化率** | **27.6%** | 定義タスク 170（未実装を含む・**最も厳しい**） |
| AI実行率 | 46.1% | 実施中タスク 102（未実装を除く） |
| AI関与率 | 82.4% | 同上（提案・下書きまで含める・**最も甘い**） |
| カバー率 | 60.0% | そもそも誰かがやっているタスクの割合 |

内訳: 自律 2 / ゲート付き実行 45 / 提案 37 / 人間 18 / **未実装 68** / 意図的にやらない 2
...
- **AI関与率 82.4%** — 「誰かがやっているタスクの8割にAIが関わっている」。**この数字だけ見ると誤解する**
- **総合自動化率 27.6%** — 「あるべきタスクのうちAIが実行しているのは4分の1強」。**これが現在地**
- **カバー率 60.0%** — 40%は**誰もやっていない**。自動化以前に未着手

つまり、**AIが関わっている領域では実際に手を動かしているが、**
**そもそも手が付いていない領域が4割ある**、というのが正しい要約。
...
|---|---:|---:|---:|---:|---|
| ③ 自律型マーケティング | **50.0%** | 68.4% | 89.5% | 73.1% | 2/11/4/2/7 |
| ⑩ AgentOps・ガバナンス | **50.0%** | 75.0% | 100.0% | 66.7% | 0/6/2/0/4 |
| ② バグ修正 | **37.5%** | 50.0% | 91.7% | 75.0% | 0/6/5/1/4 |
| ⑤ AI予算・トークン管理 | **33.3%** | 62.5% | 87.5% | 53.3% | 0/5/2/1/7 |
| ⑪ データ・プライバシー | **33.3%** | 50.0% | 83.3% | 66.7% | 0/3/2/1/3 |
| ⑫ 事業継続性 | **33.3%** | 60.0% | 100.0% | 55.6% | 0/3/2/0/4 |
| ⑥ アプリ運営意思決定 | **23.1%** | 37.5% | 87.5% | 61.5% | 0/3/4/1/5 |
| ① 次期機能開発 | **21.4%** | 33.3% | 100.0% | 64.3% | 0/3/6/0/5 |
| ④ 自動本番デプロイ | **21.4%** | 30.0% | 70.0% | 71.4% | 0/3/4/3/4 |
...
1. **③マーケティングと⑩AgentOpsだけが50%。**他は全部それ以下。
   マーケが高いのは毎朝の記事ループが実際に回っているから。AgentOpsが高いのは
   2026-08-22に集中実装したから（運転台帳・権限表・自己修復）。
2. **⑦法人経営は総合0.0%・カバー率15.4%。**13タスク中11が未実装。
   「構想中心」という外部レビューの評価が、そのまま数字になっている。
3. **⑬アナログ領域の総合0.0%は正常。**7タスクが `human_only` で、
   これは失敗ではなく**設計**（現地設営・採用判断・法的責任・危機対応）。
   ただし**AIが担える側（候補選定・書類作成・リード選定など）が全部未実装**なので、
   カバー率61.5%のうち伸ばせる余地は残っている。
4. **AI関与率は10領域中5つで100%または90%超。**
   「AIが関わっていない領域はほぼ無い」が、「AIが実行まで持っている領域は少ない」。
...
総合 **50.0%** ／ 実行 68.4% ／ 関与 89.5% ／ カバー 73.1%
...
| **未実装** | トレンドレーダー（Googleトレンド/はてブ/ランキング） | プロンプト文面まで。実組み込み未了<br>`docs/trend-radar-prompt.md` |
...
| 提案のみ | PR原稿の事実・数値・引用元の検査 | 本セッションで実施（旧アプリ名・起動時間・廃止済みトライアルを検出）。自動化されていない<br>`docs/pr-autopilot-2026-09.md` |
...
総合 **50.0%** ／ 実行 75.0% ／ 関与 100.0% ／ カバー 66.7%
...
| **未実装** | バックアップ切替の演習 | 観測はできるが意図的に落として確かめたことは無い |
...
総合 **37.5%** ／ 実行 50.0% ／ 関与 91.7% ／ カバー 75.0%
...
| 提案のみ | 誤修正率・再発率・MTTRの計測 | 変更失敗率は算出可。MTTRは検知時刻が無い<br>`data/autopilot-runs.json` |
| ゲート付き実行 | 基盤故障の検知と自己修復 | 2026-08-22実装。レーンF<br>`scripts/autopilot-selfheal.mjs` |

### ⑤ AI予算・トークン管理

総合 **33.3%** ／ 実行 62.5% ／ 関与 87.5% ／ カバー 53.3%
...
| **未実装** | 異常消費の検知 | 実績1点しか無く、何を異常とするかの基準が作れない。2週間の実測待ち |
...
| **未実装** | 保持期間の定義と自動削除 | 何をどこに保持しているかの棚卸しが先 |
...
総合 **33.3%** ／ 実行 60.0% ／ 関与 100.0% ／ カバー 55.6%
...
| **未実装** | 障害訓練 | 訓練の前に、障害を観測する仕組みが要った（2026-08-22に実装） |
...
| **未実装** | 遠隔操作できるFeature Flag | アプリ本体に無い |
...
| **未実装** | 失敗機能の自動停止とバックログ差し戻し | Feature Flag待ち |
...
| **未実装** | 証明書・APIキー・Provisioning Profile の期限監視 | 切れた日に初めて分かる |
| **未実装** | 段階公開への自動昇格 | Hard Ruleの手前 |
| **未実装** | Crash-free率・送信成功率による自動停止 | アプリ本体にRemote Feature Flagが無いため、止める手段そのものが無い |
| 提案のみ | Remote Feature Flag と緊急Kill Switch | APIには *_KILL_SWITCH / *_DRY_RUN / *_DAILY_CAP の様式がある。アプリ本体は無い<br>`../simplememo-api/src/env.ts` |
...
タスク数で数えると同じ領域が37.5%（②バグ修正）になる。
**どちらも正しく、測っているものが違う。**

- 変更行 = **やった仕事の量**のうちAIが書いた割合
- タスク数 = **やるべきことの種類**のうちAIが実行している割合

プレスリリースに出すなら**両方**を出す。片方だけだと必ず誤解される。

---

## 4. 総合自動化率を上げるには何が効くか

未実装68件のうち、**1つの実装で複数が動くもの**を挙げる。

| 実装 | 動く未実装タスク | 領域 |
|---|---:|---|
| **アプリ本体のRemote Feature Flag** | 4件 | ①失敗機能の自動停止 / ②カナリア・自動ロールバック / ④Crash-free率による自動停止 / ④段階公開の自動昇格 |
| **ASC（App Store Connect）データ接続** | 4件 | ③ASCデータ連携 / ⑥売上照合 / ⑥CAC・LTV統合 / ⑨解約理由分析 |
| **問い合わせ基盤** | 4件 | ⑧自動分類 / ⑧回答・返金案内 / ⑧CSAT / ⑧重大案件の移管 |
| **タスク単位の予算とモデル別内訳** | 3件 | ⑤タスク予算 / ⑤モデル別内訳 / ⑤モデルルーター |
| **データ棚卸し（何をどこに保持しているか）** | 3件 | ⑪保持期間 / ⑪アクセス履歴 / ⑪第三者SDK監査 |
| **障害の検知時刻の記録** | 3件 | ②MTTR / ⑫Circuit Breaker / ⑫障害訓練 |

**Remote Feature Flag が引き続き最優先。**①と②の閉ループが閉じない唯一最大の理由で、
しかも④の2件も同時に動く。

**⑦法人経営（11件未実装）は、1つの実装では動かない。**
証跡の形をオーナーが決めるところからで、技術的な積み残しではない。
