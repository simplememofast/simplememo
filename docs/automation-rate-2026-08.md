...
| 指標 | 値 | 分母 |
|---|---:|---|
| **総合自動化率** | **38.7%** | 定義タスク 173（未実装を含む・**最も厳しい**） |
| AI実行率 | 55.4% | 実施中タスク 121（未実装を除く） |
| AI関与率 | 84.3% | 同上（提案・下書きまで含める・**最も甘い**） |
| カバー率 | 69.9% | そもそも誰かがやっているタスクの割合 |

内訳: 自律 8 / ゲート付き実行 59 / 提案 35 / 人間 19 / **未実装 52** / 意図的にやらない 2
...
- **AI関与率 84.3%** — 「誰かがやっているタスクの8割にAIが関わっている」。**この数字だけ見ると誤解する**
- **総合自動化率 38.7%** — 「あるべきタスクのうちAIが実行しているのは4割弱」。**これが現在地**
- **カバー率 69.9%** — 3割は**誰もやっていない**。自動化以前に未着手
...
| ① 次期機能開発 | **46.7%** | 53.8% | 100.0% | 86.7% | 3/4/6/0/2 |
...
| ⑦ 法人経営 | **0.0%** | 0.0% | 75.0% | 30.8% | 0/0/3/1/9 |
| ⑬ アナログ領域 | **0.0%** | 0.0% | 12.5% | 61.5% | 0/0/1/7/5 |

### この表から読めること

1. **③マーケティングと②バグ修正が先頭。**マーケは毎朝の記事ループが実際に回っているから。②はカナリアガード・監視棚卸し・独立2モデル監査が2026-08-22〜23に入った分。⑩AgentOpsも同水準（運転台帳・権限表・自己修復・切替演習）。
2. **⑦法人経営は総合0.0%のまま。**13タスク中9が未実装。ベンダー登録簿ができてカバー率は15.4%→30.8%になったが、**AIが実行に回った項目はゼロ**。「構想中心」という外部レビューの評価は変わっていない。
...
総合 **46.7%** ／ 実行 53.8% ／ 関与 100.0% ／ カバー 86.7%
...
総合 **0.0%** ／ 実行 0.0% ／ 関与 75.0% ／ カバー 30.8%
