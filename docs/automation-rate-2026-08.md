...
| 指標 | 値 | 分母 |
|---|---:|---|
| **総合自動化率** | **37.2%** | 定義タスク 172（未実装を含む・**最も厳しい**） |
| AI実行率 | 53.8% | 実施中タスク 119（未実装を除く） |
| AI関与率 | 84.0% | 同上（提案・下書きまで含める・**最も甘い**） |
| カバー率 | 69.2% | そもそも誰かがやっているタスクの割合 |

内訳: 自律 6 / ゲート付き実行 58 / 提案 36 / 人間 19 / **未実装 53** / 意図的にやらない 2
...
- **AI関与率 84.0%** — 「誰かがやっているタスクの8割にAIが関わっている」。**この数字だけ見ると誤解する**
- **総合自動化率 37.2%** — 「あるべきタスクのうちAIが実行しているのは4割弱」。**これが現在地**
- **カバー率 69.2%** — 3割は**誰もやっていない**。自動化以前に未着手
...
| ① 次期機能開発 | **42.9%** | 50.0% | 100.0% | 85.7% | 2/4/6/0/2 |
...
1. **⑩AgentOpsと③マーケティングが先頭。**AgentOpsは2026-08-22に集中実装した分（運転台帳・権限表・自己修復・切替演習・資格情報の期限監視）。マーケは毎朝の記事ループが実際に回っているうえ、**広報そのもの（事実検査・主張検査・ヒーロー画像）が同日に機械化された**から。
2. **⑦法人経営は総合0.0%・カバー率30.8%。**13タスク中9が未実装。ベンダー台帳を作って提案3件が立った分だけ動いたが、**AIが実行しているタスクはまだ1件も無い。**「構想中心」という外部レビューの評価は、数字としてはまだ有効。
...
総合 **42.9%** ／ 実行 50.0% ／ 関与 100.0% ／ カバー 85.7%
