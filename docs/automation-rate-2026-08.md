...
| 指標 | 値 | 分母 |
|---|---:|---|
| **総合自動化率** | **30.6%** | 定義タスク 170（未実装を含む・**最も厳しい**） |
| AI実行率 | 48.6% | 実施中タスク 107（未実装を除く） |
| AI関与率 | 83.2% | 同上（提案・下書きまで含める・**最も甘い**） |
| カバー率 | 62.9% | そもそも誰かがやっているタスクの割合 |

内訳: 自律 2 / ゲート付き実行 50 / 提案 37 / 人間 18 / **未実装 63** / 意図的にやらない 2
...
- **AI関与率 83.2%** — 「誰かがやっているタスクの8割にAIが関わっている」。**この数字だけ見ると誤解する**
- **総合自動化率 30.6%** — 「あるべきタスクのうちAIが実行しているのは3割」。**これが現在地**
- **カバー率 62.9%** — 4割弱は**誰もやっていない**。自動化以前に未着手
...
| ③ 自律型マーケティング | **50.0%** | 65.0% | 90.0% | 76.9% | 2/11/5/2/6 |
...
3. **⑬アナログ領域の総合0.0%は正常。**7タスクが `human_only` で、これは失敗ではなく**設計**（現地設営・採用判断・法的責任・危機対応）。ただし**AIが担える側が全部未実装**なので伸ばす余地は残る。
...
総合 **50.0%** ／ 実行 65.0% ／ 関与 90.0% ／ カバー 76.9%
...
タスク数で数えると同じ領域が50.0%（②バグ修正）になる。**どちらも正しく、測っているものが違う。**
...
1つの実装で複数の未実装が動くもの（Remote Feature Flag / ASC接続 / 問い合わせ基盤）を先に解く。
