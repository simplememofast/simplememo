...
## S-20260907-fixed-but-unconfirmed

- **媒体**: X / ih / note
- **分類**: autopilot
- **一行の主張**: 直す変更はもうマージされていたのに、台帳は2件の故障を「未修理」として毎朝出し続けていた。足りなかったのは修理ではなく、それが効いていることを示す実行のほう。
- **引用できる数字**
  - 未修理として上がっていたのは2件（`ap-20260905-actions-33959414641` / `ap-20260906-actions`・出典: `data/autopilot-runs.json`・2026-09-07 時点）
  - 直す変更は 2026-09-05 と 2026-09-06 に main へ入っていた（PR #943 / #973 / #976・出典: git log の `a91a23b8` / `a0425806` / `e320d394`）
  - 09-06 の実行が止まったのは 2026-09-05T23:22:58Z で、そのうち2本が main に入る約1時間前（出典: `data/autopilot-runs.json` の `failed_at`）
  - その実行は 251 ターンで打ち切られている（出典: 同ファイルの `failure_reason`）
  - 修理済みの判定条件は「`repair_of` で名指しされていること」の1つだけ（出典: `scripts/autopilot-selfheal.mjs`）
- **note向け**: 自動運転の台帳は「修理済み」を、コードの状態ではなく記録の形で判定している。あとから読む人が確かめられるのは記録だけだから、その設計は正しい。ただし副作用がある。原因を直すPRが先にマージされ、それを踏んだ実行がまだ一度も無いとき、台帳は「未修理」と言い続ける。これは誤報ではない。直っているという主張を、誰もまだ確かめていないという意味である。今回の2件はまさにその状態だった。直った証拠として使えるのは、修正後に実際に最後まで走った実行1回だけで、それが出るまで7日近く空いた。修理の完了と、修理の確認は、別の作業として日程に載せたほうがいい。
...
- seed_id: S-20260923-note-to-email
- 媒体: note / X / reddit
- 分類: readers
