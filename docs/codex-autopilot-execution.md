...
**予約保存の確認状況（2026-09-07 11:56 JST）:** このMacのローカル予約記録では、
下表のIDに一致する予約（旧IDとの対応を含む）も実行履歴も0件で、対応する設定ファイルも
見つからなかった。予約ツールのviewはカード表示のみで、保存された設定を返していない。
したがって、下表は移管先の設計であり、現在の定期運転が稼働している証拠ではない。
予約の復旧方法は確認中。旧Claude予約の再有効化や同役割の追加予約で迂回しない。
