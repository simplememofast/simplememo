...
このスクリプトが返す `id` と `missing` を使う。`type: pr_release`、`status: running`、`evaluation_at` がJSTの当日以前で、評価未完了のものが対象になる。部分転記済みも全項目取得済みも、running の間は残る。空配列なら指標取得・台帳編集・PR作成を行わない。タスクが接続点検を指示している場合は、その結果だけをローカルへ残す。
...
`scripts/pr-evaluation-due.mjs` は評価未完了の行を返し、`capture_state` と `next_step` で処理を分ける。旧実装の「1項目でも転記すると監視対象から消える」挙動は2026-09-07に修正した。列挙は書き込みの許可でも、値の正しさの証明でもない。

- `uncaptured`: 通常の取得へ進む。
- `partial`: `existing_fields` の既存値について対象・期間・単位・証拠を照合した後、`missing` の不足分だけ取得する。既存値を再取得値で上書きしない。
- `ready_for_evaluation`: 必須4項目の数値形式が揃った状態。出典・期間の確認は別途必要で、確認後に評価と3台帳の整合確認を行う。任意の送信者比率がnullでも無期限に再取得しない。
- `invalid`: `invalid_fields` の既存値に形式・範囲の問題がある。証拠を確認するまで修正・評価・完了に進まない。比率の50を0.5へ自動変換しない。

並行作業との競合は、§6の最新main・SHA照合で防ぐ。最新mainで対象がevaluated/cancelledへ変わった場合は処理を終了する。保存前に既存値が変わっていた場合は再照合し、手元の古い差分を適用しない。
