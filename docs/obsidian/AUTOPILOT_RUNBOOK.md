...
## §0-3 一時的な確認 — `list_triggers` を1回だけ叩く（期限は日付で置かない・下の「期限」を読む）
...
**一時的な節。**1回でも `ROUTINE_MCP_PROBE:` の行が出たら、
この節ごと削除する（`ROUTINE_MCP_PROBE` で検索）。
結果は `data/automation-coverage.json` の ⑩ の行へ書く。

**日付での期限は置かない。**08-29 → 08-31 → 09-05 と3回延ばして、
**3回とも同じ理由で何も届かなかった**（下の [2026-09-05] に実測がある）。
日付を4つ目に書くと、また同じ日が来るだけで、**期限が仕事をしているように見える。**
閉じるのは次の2つのどちらかで、**どちらも日付では起きない**:

- `ROUTINE_MCP_PROBE:` の行が1本出る（＝答えが届く）
- **オーナーが仕掛けを変える**（毎日成功している別の CCR routine の
  プロンプトへ移す。`update_trigger` が要るのでエージェントからは行わない）
...
> **[2026-09-05] 期限日。答えは出ていないが、「答えが無い」ではない。**
> **答えを出せる経路が、出せない状態にある。**Routines API を直接見て確かめた
> （`list_triggers`・2026-09-05 04:45Z）:
>
>     trig_01ESF9AHax6buS9X1pdFv657  ccr-0920（この節を実行する唯一の経路）
>       last_run  FAILED
>       fired     2026-09-05T00:20:16Z → finished 00:20:28Z（**12秒**）
>
> **12秒は、Runbook を読む前に死んでいる。**この節に到達していない。
> 同じ発火で `PR実験の D+14 取得`（00:05Z）と `obsidian-community v4`（02:31Z）も
> FAILED で、`data/routine-runs.json` の open_findings が理由を記録している ——
> **session limit / resets 3am (UTC)。主系と利用枠を共有しているため、
> 副系は独立した容量の代替になっていない。**
>
> **[届かない理由は2つあり、掛け合わさっている]**
>
> 1. **配達条件そのものが不利。**上の [2026-09-01] が書いたとおり、答えが
>    リポジトリに残るのは **ccr-0920 がその日の仕事を実際にした回**（＝主系が
>    当日ロックを取らなかった日）だけ。主系が動いている限り ccr-0920 はスキップし、
>    スキップ回は `AUTOPILOT_LOG.md` を書かない。
>    **つまりこの節は「主系が落ちた日」にしか答えられない。**
> 2. **その「主系が落ちた日」は、枠が枯れている日でもある。**両者は同じ枠を
>    共有しているので、主系が枠切れで落ちた日は ccr-0920 も落ちる。
>    **答えが必要な条件と、答えられない条件が同じ。**
>
> **だから3回の延長（08-29 → 08-31 → 09-05）が何も生まなかった。**
> 4回目を同じ仕掛けのまま置くと、同じ理由でまた何も起きない ——
> **待っていれば届くもの、として扱わない。**
>
> **[仕掛けを変えるなら、材料は在る]** スケジュール起動の CCR で
> **毎日成功している routine が別に2本ある**（`7 3 * * *` の取りこぼし監視・
> `30 3 * * *` の x-engagement 昼スロット。どちらも 2026-09-05 に SUCCEEDED）。
> 同じ問い（スケジュール起動セッションが `claude-code-remote` の MCP を持つか）は
> **そちらでも同じだけ答えられる。**ただし2本とも Runbook を読まない別プロンプトなので、
> **この節からは手が届かない** —— 差し込むには `update_trigger` で
> プロンプト側を変えることになり、**それはオーナーの操作。**ここでは提案に留める。
>
> **この節は消さずに残す。**消すと「答えが出なかった」が「問いが無かった」に化ける。
> ⑩「実行の完全記録（副系）」は `nobody` のままで正しい ——
> `data/routine-runs.json` は 0.0 日前の写しを持っているが、
> **取り直しているのは人が起こしたセッションで、自動になっていない。**
...
`checkpoint.json` の `base_sha` をoriginから取得し、そのコミットで新しい空のworktreeを作る。
現在のmainにある保存器を使って、次の形で復元する。

```bash
node <mainのcheckout>/scripts/autopilot-checkpoint.mjs --restore <artifactのディレクトリ> --workspace <新しいworktree>
```

復元器はHEAD、元の宣言、差分hash、対象パスを照合し、未追跡ファイルを含めて
作業が残るtreeやsymlinkへの復元を拒否する。差分はindexへ入り、`git diff --cached` で確認できる。
再開できるのは既存の実行・予算・停止・重複防止の条件を満たす場合だけ。
自動の再試行回数や金銭権限は増やさない。元の価値契約を書き換えず、必要な台帳・生成物を
実際の再開経路に合わせて整え、通常のCIを通す。元の失敗と費用の記録を消さない。
...
**⚠ 実行はここでよいが、コミットは §2-1 の宣言より後に回す（2026-09-06 追加）。**
`decision-ci` の `verifyHistory` は「**宣言ファイルを足したコミットまでに、
宣言ファイル以外の差分が1つでもあれば落とす**」。2026-09-06、この記帳の
**`last_seen_jst` の1行**が宣言より前にコミットされていたために、
その日の全作業が `implementation preceded declaration` で落ちた（PR #969）。

**効くのは「最後の」宣言。**`--retire` で取り下げて宣言し直した日は、
**やり直す前に積んだコミットも全部「宣言より前」**に数えられる。

手順:

```
node scripts/health-intake.mjs                        # 実行して行を読む（レーンFの候補はこれで見える）
git stash push -- data/autopilot-actions.json         # §2-1 は clean tracked tree を要求する
                                                      #（--select も --retire も）
… §2-1 の契約フロー（宣言・取り下げ・再宣言）…
git stash pop && git add data/autopilot-actions.json  # 最後の宣言を push した後に記帳をコミット
```

**stash を失っても困らない。**`last_seen_jst` は時刻ではなく**日付**なので、
同じ日に `health-intake.mjs` をもう一度回せば同じ結果になる（冪等）。
...
- **レーンE（Coverage・まとめサイト化）**: `growth/content/coverage-queue.json` の
  ...
  **先頭候補の固有価値を現在の実行環境で検証できない場合（2026-09-11追加）:**
  品質ゲートを弱めず、その候補は `pending` のまま残す。候補ID、必要な検証、
  実測した不足条件、確認日をその回のローカル実行記録へ残し、`pending` をキュー順に
  走査して、**同じ回に固有価値を実測できる最初の候補**を1件だけ評価する。
  たとえば Obsidian GUI が無い環境では、GUI操作が固有価値のC13〜C17を名前だけ変えて
  毎日再試行せず、公式一次資料だけで固有価値を完結できる候補まで進めてよい。
  同じ実測済み不足条件（例: GUI不在）は同じ走査内で判定してよいが、別の未知の前提を
  新たに調べる必要が出たら、そこで走査を止める。走査結果と停止した条件を記録して
  実行可能候補が無ければその日は実装しない。先頭候補を `done` / `dropped` に変更しない、
  未検証の操作を「公式確認」で代用しない、記事のURLや主題が既存面と衝突する場合は
  従来どおりレーンA-1へ切り替える。環境や前提が変わった日は先頭から再評価する。

  **[2026-09-19訂正] 走査を止める条件は「同じ不足条件か」ではなく「どう判定できるか」。**
  上の段落は「同じ実測済み不足条件（例: GUI不在）は同じ走査内で判定してよいが、
  **別の未知の前提を新たに調べる必要が出たら、そこで走査を止める**」と書いていた。
  **この止め方が 2026-09-15 に一日ぶんの出荷を落とした**（`ap-20260915-actions-codex-01a0a1b9`・
  `no_artifact`）。その回は C13 → C14 → C15 と正しく走査し、C15 の固有価値である事前計測が
  `brand-2026-08-11-entity-merge` の全サイトスコープで拒否された。実験を避けて候補名だけ
  変えなかったのは**正しい**。誤っていたのはその次で、**「GUI不在とは別の前提だから」という
  理由だけで走査を止め、C17 以降を1件も見なかった。**

  **実験スコープによる計測拒否は「未知の前提」ではない。**
  `growth/experiments/experiments.json` と `node growth/scripts/check-experiments.mjs` で
  **その場で確定する**。外へ調べに行く必要が無い判定を「新たに調べる必要が出た」に数えると、
  **リポジトリの中だけで答えが出る壁のたびに走査が止まる。**分け方はこう:

  - **走査を続ける（次の候補へ進む）** … リポジトリ内のデータだけでその回のうちに確定する
    不適格。実験スコープによる計測拒否・`collides_with` の衝突・実測済みの環境不在（GUI不在など）。
    **件数の上限は置かない** — 確定できる判定は何件続けてもコストが増えないため
  - **走査を止める** … 外部の未知（新しいアプリの取得可否、第三者サイトの掲載有無、
    オーナー判断が要る権限）を**新たに調べないと**適否が決まらないもの

  **走査の記録はリポジトリに残す。**上の段落は「その回のローカル実行記録へ残す」としていたが、
  **09-15 の走査 C13→C14→C15 はローカルにしか残らず、翌日以降からは読めない。**
  記録が読めなければ、次の回は同じ順で同じ壁に当たる。status JSON の `reason` と
  `AUTOPILOT_LOG.md` の当日エントリに、**候補ID → 不適格の種別 → 続けた／止めた**を
  1行ずつ残すこと。

  **ゲートは弱めない。**この訂正が変えるのは**走査を止める条件だけ**で、
  品質80点・§28・固有価値・実験非干渉のどれにも触れていない。拒否された候補は
  `pending` のまま残り、`done` / `dropped` にしない。
...
4. 出力された `data/decision-intents/<id>.json` だけを**別コミットにして先にpush**する。**この時点までに宣言ファイル以外のコミットが1つでもあると `decision-ci` が落ちる**（§2 レーンFの記帳を含む。2026-09-06 に実際に落ちた）。この時点では記事やコードを編集しない。CIがコミットの順序を検査し、事前データから予測・適格性・較正の参照記録を再計算する。参照が無い・古い・後から変更された新規契約は通らない。この配線より前に宣言済みの契約は当時の形式で検証し、過去に参照記録を付け足さない。
...
**2026-09-15再確認:** 上の取得待ちは8/11時点の履歴。現在は既存SEO Dailyの検証済み結合データを再利用できる。[R4/R5の今回の判断](QUERY_PAGE_REVIEW_2026-09-15.md)は既存ページの維持であり、C19も独自価値・実機検証等の条件が残る。古い注記から手動CSVを再依頼したり、欠測をゼロにして統合を進めたりしない。
...
- 主系の終了検査は、モデル開始前のブランチと、今回の実行IDを持つPR上の新しい記録を照合する。宣言だけのブランチや当日statusは成果物の証明にならない。PR待ちは本番出荷と区別する。取得失敗や照合中の更新は「確認不能」として調査へ残し、「成果物ゼロ」と断定しない。再実行で以前のPRを更新する場合も、今回の結果を記帳する（以前と全く同じ記録は新しい結果にならない）。
...
**運転台帳を同期してから、同じ監視の中で実費を取る。** 2026-09-05の実測で、最初の監視が故障runを記録した後に実費の未記録を発見し、次の監視まで費用が載らない形が見つかった。実費処理は他のハンドラの後に台帳を読み直して1回だけ実行し、修理や封じ込めを繰り返さない。新しく見つかった実費処理にも従来の権限・適格性判定を掛ける。記録後は実費ゲートも再計算し、取得失敗は0円や解決にしない。
...
2026-09-07の所有者判断で、開始前想定の5倍以内の費用判断はAIへ委任された。
`--ack-overrun <run_id> --by ai --projected-usd <総費用見込み> --why "判断根拠"`
でレビューできる。正典は [費用委任の手順](../cost-delegation.md)。
5倍を超える見込みは追加支出前に所有者へ上げる。月次枠・種別枠・SDKの停止閾値は維持する。
開始前想定が残っていない過去runを、後付けの想定で解除しない。
閉じ条件 `budget_overrun_reviewed` は台帳のレビューまたは超過解消を確認するもので、
実行の成功や自律復旧の証拠ではない。
