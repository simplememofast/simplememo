...
**予約保存は未確認:** 2026-09-07 11:56 JSTのローカル記録では、上記IDの保存済み予約と
実行履歴が存在しなかった。以下の時刻・IDは移管設計であり、稼働済みとは扱わない。
保存状態を読み戻してから実タスクの自然起動を確認する。確認方法と復旧状況は
現行の実行契約を参照し、旧系の停止は維持する。
