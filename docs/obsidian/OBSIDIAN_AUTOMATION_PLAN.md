...
**2026-09-15の現在地:** クエリ×ページは既存SEO Dailyで取得し、Company readerが原本・期間・品質を検証している。[R4/R5の再確認](QUERY_PAGE_REVIEW_2026-09-15.md)では既存面の維持を選択した。以下の「データ無し／オーナー作業」は旧計画時点の記録であり、現在の定期取得状況ではない。ページ集計だけでクエリの着地を推測しない。
...
候補クエリ
 ↓
同じ検証済みスナップショットの queryPages から、そのクエリの観測済み着地を探す
 ↓  （結合行が無ければ不明。表示下限・匿名化があり、不在は需要ゼロではない）
観測済みの既存着地あり → Refresh / Expand 候補として既存ゲートで評価
欠測・下限未満・未検証 → 不明のまま保留し、既存ページ・内容の一覧と照合
別途の調査で未カバーと固有価値を確認 → 元の需要・衝突・実験ゲートを通った場合だけNew候補へ
```
