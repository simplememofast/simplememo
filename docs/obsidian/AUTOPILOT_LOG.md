...
- `autopilot-drill --check`: 新コード `run_takeover` を含め **gate の16コード全てに演習
  シナリオがある**（35→39。引き継ぐ／作業があれば追い越さない／90分未満は待つ／
  読めなければ追い越さない）
...
## 2026-09-03（2） — 保守のみ（1回上限を実測から引き直した）

- **判断根拠:** 08-25 の記録が2つ書いていた。①「上限を上げるのではなく1回のコストを下げる」
  （オーナー判断・承認したのは article だけ）②「**節約後の実測が出たら、そこから引き直すのが筋**」。
  同じ注記が repair についてこう予告している —— 「実測は $11.9329 で同じ問題を抱えているが、
  承認されたのは article だけ。**次に repair が走ると同じ形で1回上限に当たる**」。
  **そのとおりになった**（09-01 $9.3726 / 09-02 $13.5739 が未レビューで主系の repair を止めた）。

  そして **$6.00 の article 上限は、実験として結論が出ている。** 旧注記はそれを
  「削減が効いたかの判定も兼ねる —— 効けば($4.50)超過は起きず、効かなければ($7.29)鳴る」と
  定義していた。**鳴った** —— 削減後の実測は 08-26 の $7.8771（162ターン）で、目標 $4.50 に届いていない。
  **上限は仮説を否定する形で仕事をした。**

- **やったこと（オーナーの指示「上限変更しましょう」を受けて）:**

  | | 旧 | 新 | 観測最大 | ターン上限まで回した暴走 |
  |---|---:|---:|---:|---:|
  | `article.max_usd_per_run` | $6.00 | **$10.00** | $7.8771 (162t) | 250t x $0.0486 ≒ $12.15 |
  | `repair.max_usd_per_run` | $3.00 | **$18.00** | $13.5739 (139t) | 250t x $0.1147 ≒ $28.75 |

  **線の引き方を「実測の何倍」ではなく「正常な run と暴走の間」にした。**上限の役割は
  次の1回を止めることなので、`--max-turns 250` まで回した場合の額を上側の基準に取れば、
  正常な run を通しながら暴走だけを掴む。

- **月次総額 $40 は触っていない。**`monthly_usd_cap` は権限表で human_only。
  ただし `check-model-routing` が「1回の上限が月次枠を超えている」で落ちるので、
  **種別枠を $40 の中で組み替えた**（合計は 40 のまま）:

      article  18 → 12      repair  8 → 18      analysis 6 → 3      pr 4 → 3

  根拠は当月の主系実測 —— **$22.9465 の全額が repair で、article は $0**。
  analysis / pr / qa_triage / audit は**一度も走っていない**。
  **ただし副系CCRの実費は観測できない**ので、この配分は主系の実測だけに基づく。

- **⚠ 未レビューだった2件は、レビューされずに閉じた。**
  上限を引き直した結果、09-01 $9.3726 と 09-02 $13.5739 は「超過」でなくなり、
  `budget_overrun_reviewed`（超過でなくなれば閉じる）で自動的に閉じた。
  台帳の根拠欄も「1回上限の超過として検出されなくなった（上限の見直しか実費行の訂正）」と書いており、
  **人がその2件を1件ずつ見た、とは書いていない。**これが正しい記録。

- **変えていないもの:** モデル割り当て（repair の opus —— 「安いモデルにやらせると外し、
  同じ故障をもう一度直すことになる」は実測なしに覆さない）／`--max-turns 250`／
  月次上限 $40 ／ `--ack-overrun` が人間のみであること。

- **検証:** `check-model-routing --check`（1回上限 ≤ 月次枠・ワークフローが実際に引いていることも確認）、
  `autopilot-budget --check`（枠の合計 = 40）、`--check-run-cap --task repair` が**通るようになった**。
  `scripts/preflight.mjs` 122本中1本失敗（既存の `check-generators --run`。main でも同じ）。

- **残っている論点:** **$40/月は、実測された日次運転を支えない。**08-25 の記録が
...
## 2026-09-03（4） — 保守のみ（散文だった「変更の間隔」を実際に検査する）

- **判断根拠:** 前の回（月次上限 $40→$280）で見つけたもの。
  `financial-policy.json` の `min_days_between_changes: 14` は 08-22 から台帳にあったが、
  **検査されていなかった。**`validate()` は「数値が入っているか」しか見ず、
  `validateApprovals()` は承認日の間隔を見ていない。つまり **同じ日に何度でも上げられた。**
  規則の note が「実費の中央値が動いたと言える最小の窓」と理由まで書いているのに、
  それを守らせるものが無い —— この台帳が繰り返し戒めている「散文は手を挙げない」の形。

  **前の回で同時に入れなかったのは、入れると同じPRの変更自体が落ちるから**
  （08-22 → 09-03 は12日）。「入れた規則を同じPRで破る」形になるので別件にした。

- **やったこと:** `validateApprovals()` が承認日の間隔を実際に見るようにした。
  `min_days_enforced_from`（= 2026-09-03）**より後**の承認だけを判定する。

  **遡って判定しない。**seq=1（08-22）と seq=2（09-03・間隔12日）は規則が機械化される前の
  記録なので対象外。**免除ではなく、適用の開始点。**規則が無かった時点の記録を、
  後から作った規則で落とすのは、記録のほうを嘘にする。
  **次の変更（seq=3）から14日が実際に効く。**

- **検証:** 自己テスト **18件（+5）**。
  「間隔が足りない承認は落ちる」／「足りていれば通る」／
  「**開始点より前は遡って判定しない**」／「**開始点の宣言が無ければ判定しない**（黙って遡及適用しない）」／
  「**実データの承認記録が、いまの規則で通る**」。
  `preflight.mjs` 123本中1本失敗（既存の `check-generators --run`。main でも同じ）。

- **残り:** A2（枠の分離＝`ANTHROPIC_API_KEY` を主系のフォールバックに）は着手していない。
  **オーナーしか置けない secret が要り、置かれるまで動かして確かめられない。**
  「止まることを確かめていない停止機構は、無いのと同じ」と同じ理由で、
  **試せない経路を先に作らない。**

---

## 2026-09-03（5） — 保守のみ（A2 を「意図的にやらない」へ移した）

- **判断根拠:** オーナー判断「`ANTHROPIC_API_KEY` ←これは追加チャージなら使わない方向」。
  API課金はサブスクとは別勘定なので、**月次上限 $280 の外に第2の請求経路ができる。**
- **やったこと:** 3か所に記録した。**空欄にして「忘れている」と読ませない**ため。
  - `docs/autopilot-roadmap.md` §4「意図的にやらないもの」に1行 ＋ **帰結の節**
  - `docs/autonomy-measures-2026-09-03.md` §5 を「提案のまま」から「やらないに決まった」へ
  - `.github/workflows/obsidian-autopilot.yml` の秘密鍵の節（**鍵を置く判断をする人が最初に読む場所**）
- **⚠ 帰結を隠さない —— 枠の単一障害点は消えないまま残る。**
  08-31 に主系も副系も同じ 429 で落ちたのは**両方が同じアカウントの枠を食う**から。
  別勘定を足さないなら**枠を分ける手段は無い。**残るのは「当たらないようにする」だけで、
  それも次の3つの制約つき:
  - 週次枠は**拒否される前に状態が読める**（`allowed` / `allowed_warning` / `rejected`）が、
    **残量（数値）は読めない** —— あと何回走れるかは言えない
  - **主系からは同じ形で読めない**（`get_session` は CCR セッションのメタデータ）。
    実装するなら**副系が観測して台帳に書き、主系が読んで縮退する**形
  - **スケジュール起動の副系が `get_session` を持つかは未確認**
    （`routine-snapshot-setup.md` が同型の問題で2経路とも閉じている）
  **「枠が尽きたら止まる」は、当面この運用が受け入れた制約。**
  止まったことは検知される（`act-shipping-outage` / `owner_direct`）が、**避けられはしない。**
- **鍵の配線そのものは外していない。**`anthropic_api_key` の入力は残っている ——
  鍵が未設定なら無害で、設定すれば課金経路が開く。**開けるかどうかはオーナーの判断で、
...
### おまけ — 検査を走らせたら、別のものが2つ動いた

**`check-generators --run` は「作業ツリーが汚れていると走らない」**（人の編集を潰さないため）。
このため 09-03 の各セッションはずっと **走らせずに「1本失敗」と記録していた。**
コミットしてから走らせたら、生成物が2件古かった:

    data/revenue-series.json    synced_at 08-28 → 09-03 / covered_days 2 → 4
    data/financial-policy.json  revenue_history_days 2 → 4（上から導出）

隣（simplememo-ios）からの写しで、判断は入っていない。**別コミットにしてある。**
（この環境で `app-releases.mjs --write` を走らせるために `simplememo-ios` を
`git fetch --unshallow` した。浅いクローンだとタグと履歴が欠けるので、
あのスクリプトは自分で拒否する。生成物の差分はそれとは無関係で、写しが古かっただけ。）

そして写しが動いたことで `check-waiting-progress --selftest` が落ちた ——
**実データの `revenue_28d` が stalled であること**を直接assertしていたから。
「この検査が空回りしていないこと」を実データで示すのが目的で、当時それが本当に止まっていた。

assert は消していない。**実データが「止まっている」ことに依存するのをやめた** ——
①実データの全件を分類できる（`unreadable` を残さない）②**その実データの鮮度だけを退かせると
実際に `stalled` が出る**（判定が生きている）、の2件に置き換えた。
変異で確かめた: `staleDays > budget` の枝を殺すと**5件が落ちる**（置き換えた1件を含む）。

⚠ **「進んでいる」を「直った」と読まない。**`revenue_28d` には
`acknowledged_stall`（since 2026-09-01 / review_by 2026-09-08）が付いていて、
「covered_days は暦日ではなく**購入のあった日**しか増えない（約3日に1回）」という診断が書いてある。
今回の実測は **6暦日で 2→4**。**その診断のとおりの速さで、直ってはいない。**
約束の期限（09-08）はまだ先なので、承認はそのまま残してある。
...
## 2026-09-03（7・代走 owner-session） — 保守のみ（週次上限の手を1つ実測で潰し、オーナーの判断を台帳に落とした）

### 判断根拠

前の回（6）が唯一オーナーへ残した依頼が `ap-20260831-ccr0920`（`usage_limit`）で、
`escalation-rules.json` は手を3つ挙げていた —— **待つ・枠を上げる・1回あたりの入力量を減らす。**
オーナーの指示は「枠を適切にあげて」。**「適切に」を決めるために先に測った。**

`get_session` / `list_sessions` の `external_metadata`（`usage.cost_usd` と `rate_limit_info`）から、
現在の週次窓の消費を数えた。結果は**3つ目の手を落とす**ものだった:

- 週次上限は **アカウント全体で共有される資源**で、同じアカウントの対話セッションも同じ枠を食う
- ある週次窓で枠を消費していたのは**圧倒的に autopilot 以外**で、autopilot の寄与はごく一部
- したがって **autopilot 側の入力量をどれだけ削っても、この上限はほとんど空かない**
- **08-30/08-31 に出荷が消えたのは、autopilot が使いすぎたからではなく巻き添え**

**金額はこのリポジトリに書かない。**`simplememo` は公開リポジトリで、この台帳は
`data/autopilot-status.json` 経由で本番に配信される。数えたのはオーナー個人の対話セッションの
消費で、載せる場所ではない（Runbook §5-5 と同じ規律）。**構造だけを残し、額は残さない。**

**オーナーの判断（2026-09-03）: ① プラン階層で対応する。**実際の消費は x20 プランに収まるという判断。
追加の課金経路（`ANTHROPIC_API_KEY`）は引き続き使わない（PR #808 の決定は変えない）。

**枠そのものはセッションから上げられない。**claude.ai の課金・プラン設定に届くツールが
この環境に無い（GitHub / BigQuery / CCR / Bash のどれも通らない）。**実行はオーナー側。**
ここでやったのは、その判断が次のセッションに伝わる形にすることだけ。

### やったこと

#### (1) `data/escalation-rules.json` の `usage_limit` に、実測と判断を追記

3つの手が**等価ではない**こと（3つ目はこの上限にほとんど効かない）と、
オーナーの判断（プラン階層・x20 に収まる）を note に書いた。

**⚠ このファイルは `self_repair.may_modify` に入っていない。**入れると `who` を owner に
書き換えて修理から逃げる経路ができるので、意図的に外してある。**今回もそこは足していない** ——

- 触ったのは `note` の文面だけ。判定に使う `who` / `stop_automation` / `channel` /
  `within_hours` は**1つも動かしていない**
- `autopilot-selfheal --selftest` の「escalation-rules.json が may_modify に入っている」検査は
  通ったまま（7件0失敗）
- **これはレーンFの自己修復ではなく、オーナーの指示による追記。**その区別を note 自身にも書いた

#### (2) `data/autopilot-cost.json` に「月次上限と週次上限は別物」を書いた

    monthly_usd_cap ($280) … トークンのAPI換算額を月で積んだ notional な上限。
                             autopilot-budget --check が主系の Gate を止める
    週次の使用量上限        … Claude サブスクのレート上限（rate_limit_info の seven_day）。
                             **アカウント全体で共有**される

**分母が違う。**月次上限を上げても週次上限は1ミリも動かない（逆も同じ）。
08-30/08-31 に出荷が消えたとき、当月実費は上限の1割にも届いていなかった。
**「AI実費 $X / 上限 $280」を週次上限の余力として読まないこと**を台帳に明記した。

ついでに、上の台帳が長く書いていた「副系CCRの実費は観測手段が無い」を**経路で分けた** ——
オーナー起動の対話セッションは `list_sessions` / `get_session` から読める（本日実測）。
**スケジュール起動（Routine）の副系が同じように読めるかは未確認**で、
`list_sessions` の既定はトリガー発火ぶんを返さない。
そして**読めるようにする＝公開面に載せる**ことでもあるので、取り込むかはオーナーの判断（金額の公開可否）。

#### (3) `act-selfheal-ap-20260831-ccr0920` を「既知の制約」へ

**done にはしていない。**依頼していたのは「人の判断」で、それは済んだ。
一方この行の閉じ条件（`no_failure_since`）は「ccr-0920 が実際に着手して usage_limit が再発しないこと」で、
**まだ満たされていない** —— 08-31 以降 ccr-0920 の行が台帳に1つも無い。
`acknowledged` は「知っていて受け入れている」で、日報では**既知の制約として表示され続ける**（消えない）。

### 本番で新Gateが初めて走った（記録）

09-03（1）が「**この配線は本番でまだ1回も走っていない。**最初の実走は明日 06:00 JST の主系」と
書いていたが、**11:14 JST の手動起動（`workflow_dispatch`・run 33706886868）で走った。**

    Gate（秘密鍵・当日重複・死んだ占有の引き継ぎ）  success   17秒
    以降の全ステップ                                 skipped

当日ぶんの作業が既に在る日なので**スキップが正しい判定**で、bash を消して
`autopilot-gate.mjs --preflight` に寄せた配線が本番で期待どおりに動いた1件目。
台帳へは日次アクチュエータの `reconcile-runs` が自動で入れた
（`ap-20260903-actions-33706886868` / `skipped_gate` / `attempted: false`）。
**セッションが手で足していない。**

### 検証

- `check-escalation --check`（規則の無い故障種別なし・自己テスト5件0失敗）
- `autopilot-selfheal --check` / `--selftest` 7件0失敗 —— **may_modify の固定が生きていることを確認**
- `autopilot-budget --check` / `autopilot-act --check` / `autopilot-runs --check`
- `scripts/preflight.mjs` 124本すべて通過
- `seo-check.js` 268ファイル 0 errors 0 warnings
- **モバイル実描画QAは実施していない。**HTML/CSS/画像の変更が1つも無い回のため（変更は data/ と docs/ のみ）

### 残る弱さ・申し送り

- **台帳は「主系が当日ロックを先に取ったので副系がスキップした」と「副系が死んでいる」を区別できない。**
  スキップ回は `attempted: false` で、行すら残らない日がある（09-01 の ccr-0920 の起動は
  09-01 の主系の行の**注記にしか残っていない**）。**副系が本当に生きているかは、主系が落ちた日にしか分からない。**
  ここは今回も直していない —— 直すには副系側に「走ったこと自体」を書かせる経路が要る
...
## 2026-09-03（9・代走 owner-session） — 配線（消費側だけ在って生産側が無かった記事ネタ台帳）

### 判断根拠

前の回（8）で見つけたもの。`docs/story-seeds.md` が **HTTP 404** で、
`x-engagement-simplememofast` の Routine が毎日そこを読みに行っていた。
オーナーの「お願い」を受けて直した。

**根本原因は「ファイルが無い」ではなかった。**`story-seeds` スキルの本文にこう書いてある:

    台帳への追記は毎日のオートパイロット（docs/obsidian/AUTOPILOT_RUNBOOK.md §5-6）が行う。

**その §5-6 が存在しない。**Runbook は §5-5（配信の種）で終わっていた。
つまり**消費側だけが3つ作られ**（`story-seeds` スキル・X の Routine の10番目の手順・
`note-simplememo-3days` の記述）、**種を書く側が一度も書かれていなかった。**
ファイルが無いのは結果で、原因は生産側の不在。

**404 は設計どおり握られていた。**スキルも Routine も「取得できない/404 は異常ではない」と
書いてあり、ログに1行残して先へ進む。**壊れてはいないが、その機能は一度も発火していない。**
静かに死んでいる経路の形として、この台帳が繰り返し書いてきたものと同じ。

### やったこと

#### (1) `docs/story-seeds.md` を作った（種7件）

**書式は発明していない。**`story-seeds` スキルが列挙している欄をそのまま使った ——
`seed_id`（`S-YYYYMMDD-slug`）/ `媒体` / `分類` / `一行の主張` / `引用できる数字` /
`note向け` / `X向け` / `英語圏向け` / `使わない表現`。

**数字は全部この場で裏を取った。**消費側は §0 で「台帳の値をそのまま引く・再計算しない」と
縛られているので、**ここが間違っていると公開投稿が間違う。**各行に日付と出典ファイルを付けた。

| seed_id | 素材 |
|---|---|
| `report-said-zero` | 出荷の2時間後に「公開記事0」と報告した日（PR #793・08:06 JST マージ） |
| `unreadable-vs-absent` | 取得失敗を「発生していない」と記録。除外6件中3件に実費があった |
| `rule-not-wired` | 規則が人へ渡すと決めた依頼が3日ぶんAI側に積まれていた |
| `issue-closed-same-day` | #591 は当日 10時間49分で閉じたのに、台帳は8日間「判定不能」 |
| `check-never-ran` | 「汚れていたら走らない」検査が、汚れたまま呼ばれ続けて一度も走らず |
| `shared-weekly-quota` | 上限で止まった日、枠を食っていたのは自動運転ではなかった |
| `unanswered-intent` | 順位8.5・128imp・クリック0 の検索語（`refresh-queue.json` A1） |

**`shared-weekly-quota` には金額も比率も書いていない。**このリポジトリは公開されており、
公開済みの値と比率を組み合わせると総額が復元できる。**その注意書きを種の中にも、§0 にも置いた。**

**機械で検査した:**

- `X向け` **7件すべて全角120字以内**（97〜109字。スキルの「そのまま出せる原稿」の条件）
- `seed_id` の書式 `S-YYYYMMDD-slug` に**全件一致**・**重複なし**
- 必須8欄が**全種にそろっている**
- `check-pr-facts --file docs/story-seeds.md` → **OK**

`fact-check: draft` を宣言した。**内部メモではなく公開投稿の土台になる原稿**なので、
配信原稿と同じ規則を全部かける。宣言しないと検査自体が「未宣言」で落ちる
（暗黙のファイル名規則にしない、という既存の設計）。

#### (2) Runbook §5-6 を新設した（生産側）

**ファイルを1回置くだけでは、今日で止まる。**種を足し続ける規則が要る。

**§5-5 との違いを最初に書いた** —— あちらは**出荷した成果物**を配信に接続する種で、
記事を出した回だけ書く。こちらは**運転で起きたこと**の種で、
**記事が出なかった回にも書ける**（むしろ壊れた回・直した回のほうが強い素材になる）。

**「書かない回」も明記した** —— 台帳の同期だけ・取り残しの始末だけ・数字の引き直しだけ。
**種が無いのは正常な結果**で、無理に作ると消費側の投稿が薄くなる。

### 検証

- `X向け` の字数・`seed_id` の書式と重複・必須欄の充足を**スクリプトで数えた**（目視していない）
- `check-pr-facts --file docs/story-seeds.md` → OK。既定の `--check`（`docs/pr-*.md`）も通過
- 種の数字はすべて出典に当たった: PR #793 のマージ時刻（GitHub API）/ Issue #591 の
  作成・クローズ時刻（GitHub API）/ `refresh-queue.json` A1 の 128imp・pos 8.46・
  期待クリック 3.4 / `data/revenue-series.json` の `synced_at`
- 正式名称は `data/site-constants.json` の `appNameJa` から取った（書き写さず、
  版・料金・無料枠は当日値を引くよう §0 に書いた）
- `check-definitions --bump noise_floor`（Runbook を触ったため。**定義は変えていない**）
- `preflight.mjs` / `seo-check.js`
- **モバイル実描画QAは実施していない。**変更は `docs/` と `data/` のみ

### 残る弱さ・申し送り

- **この経路もまだ実走していない。**次に X の定期タスクが走る回（昼12:30 / 夜19:30 JST）に
  初めて 200 が返る。**引けたか・種を使ったかは消費側の報告にしか出ない**ので、
  そこを見ないと成功は確認できない
...
## 2026-09-06（owner-session・PR #1024） — C09 Capacities比較を別の宣言から復旧

- run_id: `ap-20260906-owner-session-capacities-recovery`。オーナーの未公開項目復旧・追加権限付与を受けた手動回で、intervention=request。定期主系の正常完走と数えない。
- 元の主系run 33996430959 / PR #969は失敗・未マージとして保持。通常再実行34022754964は当日ブランチの既存作業を検出してスキップしたため、force再実行や旧ブランチの削除をせず、新ブランチで既存C09を復旧した。
- `kind: article`を欠く候補は、費用予測20ドルをanalysis上限1.5ドルで判定されて不適格になった。記事種別を明示して再選定し、予測額を小さく見せる修正はしなかった。選定宣言を31c74dbcで実装前にcommit/pushし、親のデータから再計算できることを確認。
- `/obsidian/compare/capacities/`を追加。Mac版1.70.2 / macOS 26.4.1でログイン前の初回デモを操作し、日次メモへの日本語入力、Bookの属性、Backlinks、リンク図を確認。初回デモであること、本文の一部が比較用文面であること、実際の同期・書き出し・移行・iOS性能が未検証であることを明示。保存方式等は公式資料の確認として別記した。
- 比較ハブ・Obsidian案内・旧/vs/capacities/から接続。旧ページにあった未実測の3〜5秒、Spacesの共同編集、オフラインの分類を訂正。C09キュー、content graph、配信seed、OG、sitemapを同期。新しいEN記事は増やしていない。CTAは関連するObsidianの案内への次の一歩を中心にし、cta-boxは置いていない。
...
## 2026-09-14（Codex定期主系・06:00）— C16 手動ジャーナリングの記録例

予約記録で予定06:00 JST、実起動06:01:34、automation `obsidian`、実タスク
`01a09c93-4c00-78c0-9d0f-f47136880182` を照合。初回ターンpreflightはrun=true。
当日claim・共有PR・未終了の別予約実行なしを確認し、宣言直前にも再確認した。
過去の修理IDだけを当日占有者として持ち越していない。

候補C16とC18を比較し、`journaling-c16-20260914`を事前宣言・push後に実装。
共有refは新規Git pushで取得（APIによるref作成は未アップロードSHAを422で拒否。
この拒否ではrefは作成されず、その後の新規push成功を確認）。force操作はしていない。
run IDは `ap-20260914-actions-codex-01a09c93-4c00-78c0-9d0f-f47136880182`、PR #1342。

C13/C14のプラグイン動作、C15のPARA Vaultは現在の実証が無くpending維持。
C16はPR470の日付Markdown往復画像と既存手動ひな形の再利用が可能と判断した。
画像は過去Linux版の限定実証で、現在のGUI・変数展開・同期の確認にはしない。
公開ひな形の200応答はGit原本とバイト一致。読者には保管庫内に日付名のノートを作り、
ひな形本文を貼る手動経路も明示した。これは第三者レビューの指摘を反映したもの。
架空の記入例と見返し表は継続効果の実測ではない。

Company OSではAppsFlyerの初回自然定期取得を原readerで検証。ASCは既存出力を再利用。
GA4は同じ暗号化要求UUIDを再開して検証し、再取得リクエストを重複作成していない。
GSC dailyの暗号化artifactは既存成功runに無くunavailableを保持し、既存の検証済み
28日exportを判断入力へ使用した。クエリ×ページの結合が無いCTR差分を、未回答意図や
原因の証拠として用いていない。Daily/Weekly/follow-upは既存主系内で実行。
Monthlyは当月の既存レビューを保持。週次AI観測は次の予定前でmodel_calls=0。
初回自然収集の既存Goal通知を予約・送信し、受信側のacknowledgementを確認した。

障害案内はobserve profileの読取1回で直近48時間・最大49行を確認（実記録12行）。
最新は2時間以内のinactive、eligible=0/errors=0。初回10:00 UTCの失敗を保持し、
修正済みの案内監視を再修理・再加点していない。activeの公開や端末到達は未観測。

記事の初稿SEOは281面で0 errors / 0 warnings。対象記事の320/390/900/1280pxは
横漏れ・画像欠落・JavaScriptエラーなし。最終の全検査とCI・配信照合はPRへ記録する。
品質評価88/100は編集判断で、検索成果ではない。契約p=.25は翌日の公開継続の不確実性を
反映し、今回の出荷を契約成功として先取りしない。Codex実費はnull、Claude実費台帳は未変更。

## 2026-09-15（Codex定期主系・06:00）— C15事前計測の実験重複で未完

予定06:00 JST、実起動06:01:09.852を予約記録と実タスク
`01a0a1b9-4982-70f3-b2d6-eed77be68e26`で照合。初回preflightは許可され、停止・月次/記事/実行上限・当日claim/共有PR・未終了予約実行を確認した。過去の修理IDは当日所有者へ持ち越していない。

現行Obsidian 1.13.7を実画面で確認。C13/C14は制限モードとプラグイン固有動作未検証を記録し、設定を変更せず保留。次のC15は合成VaultのPARA・チェック項目・リンク・Archive移動の既存証拠を確認し、グラフ証拠と合わせ45ファイルの原本SHAが一致した。完全なCODE実践や生産性の効果は未実証のまま。

Company候補はcontent:ai-visibility-gap（優先度35.7550875）と第三者言及の読取調査（31.8990375）を比較。C15の事前計測は、`brand-2026-08-11-entity-merge`の全サイトスコープで拒否された。同じスコープの他ページへ候補名だけ変えて回避せず、保護された計測ゲート・実験範囲・承認済み指標を変更していない。第三者候補2URLの本文取得もWeb readerはtimeout、通常HTTP読取は403で、掲載有無の結論は出していない。

宣言・本文実装・記事公開はなし。初回許可後のため`attempted:true / no_artifact`、停止した層はeligibility・blockedとして残す。この記帳PRは通常のCodexブランチから運転記録と既存公開運転表を同期するもので、共有の事業施策claim、事業施策の出荷、Company finishではない。Company receiptは未完のまま私的canonical stateに具体的な拒否理由を保存した。

日次は既存GSC暗号化artifactを検証して再利用、AppsFlyerは原reader、ASCは既存出力を再利用。GA4は同じUUIDで再開し検証した。Bingは2026-W38の手動画面観測を再取得せず週次付録へ反映し、API未接続は独立の欠測として保持。週次AI観測は予定前でmodel calls=0。Daily/Weeklyとfollow-upを実行し、月次は当月の既存評価を保持。未成熟実験の成功は先取りしていない。
