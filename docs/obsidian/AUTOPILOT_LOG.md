...
## 2026-09-01（主系・GitHub Actions） — レーンF。死んだ占有が1日ぶんの出荷を消す穴を塞いだ（記事なし）

### §0 冪等性・占有

`emergency-stop.json` は `stopped: false`（経路別も全部 false）。当日ブランチ無し・本番status
JSONは 2026-08-29・当日head PR無し。`claude/obsidian-auto-20260901` を空コミットで占有し、
非fast-forward拒否なく通過（他経路の同時着手なし）。

**3日ぶりに主系が動いた。** 08-30・08-31 は同じトークンで HTTP 429（週次の利用枠）に当たって
即死しており、上限は 2026-08-31T23:00Z にリセットされている。**本日そのトークンで通っていること
自体が、鍵が無事だったことの実測**になっている。

### レーンF — 未修理3件のうち、直せるものと、すでに直っていたもの

`autopilot-selfheal.mjs` が3件を上げた。**うち2件は、修理そのものは済んでいて記録だけが
無かった。**

| 対象 | 種別 | 実際の状態 |
|---|---|---|
| `ap-20260829-ccr0920` | `claim_without_completion` | **未修理。本日直した** |
| `ap-20260830-actions` | `usage_limit` | 応答経路は PR #738 で修理済み・記録が無かった |
| `ap-20260831-actions` | `usage_limit` | 同上 |

### 直したもの — 占有は「ブランチが在る」だけで成立していた

2026-08-29、副系(09:20)が当日ロックを claim だけ取って、記事もPRも作らずに終わった。
主系の Gate はそのブランチの存在を『進行中/実行済み』と読んで3秒でスキップし
（run 33230445898・steps 3〜13 が全て skipped）、**その日は誰も走らないまま全経路が緑になった。**

**主系側の行は `skipped_gate` として残る。**つまり「重複でスキップした正常な日」と
見た目が区別できない。占有は二重着手（PR #521/#522）を止めるために入れたものだが、
**占有した経路が死ぬ場合を考えていなかった。**

死んだ占有だけを引き継ぐ判定を入れた。条件は3つで、**1つでも欠けたら従来どおりスキップする**:

1. claim 以外の仕事が載っていない（`ahead_by <= 1`）— 書いている最中は claim 以外の
   コミットが載る。**「古い＝死んでいる」ではない**
2. 最新コミットから 120 分以上動いていない — 主系の `timeout-minutes: 90` より長く取る。
   **生きている経路を横から奪わないほうが先**で、遅れて取り戻すのは二番目でよい
3. 1と2が実際に測れた — **「測れなかった」は「古い」ではない**（§1-2 の「取得できなかった」と
   「増えていない」を混ぜない原則を、占有の判定にも当てる）

引き継ぐときは**同じブランチに引き継ぎコミットを積んでから作業する。**切り直さない・
force しない。積むこと自体が後発への通知になる（claim だけでなくなるので、後から来た経路は
条件1で止まる）。
...
08-30・08-31 の 429 について、**応答経路の修理は 2026-09-01 08:09 JST のオーナーセッションが
PR #738 で出荷済み**だった（429 を「資格情報が通っていない・`setup-token` を再実行せよ」と
断定していた2値の切り分けを3値にした。**無事な鍵を捨てる指示になっていた**）。
それでも selfheal に上がり続けたのは、`repair_of` を書いた行が無かったから。

**直接の原因は `autopilot-runs.mjs --append` に `--repair-of` が無かったこと。**
Runbook §2 は「直したら `repair_of` を書く。書かない限り翌日も未修理として上がる」と
定めているのに、**その書き口が無く手作業でしか書けなかった。**手作業でしか書けない欄は
忙しい日から落ちる —— 記録ではなく善意に頼った運用になる。`--repair-of` / `--repair-note` を足した。

**使用量上限そのものはリポジトリ側の変更では防げない。**アカウントの週次上限で、
2026-08-31T23:00Z にリセットされた。`repair_note` にそう書いてある —— 修理が及ぶのは
応答経路までで、上限そのものではない。ここを曖昧にすると「直した」が実態より強くなる。

### 検証

- **本番APIで実走**: compare API で当日ブランチを測り `ahead=1 / 経過8分` →「生きている＝
  引き継がない」、存在しないブランチでは ahead が空 →「測れない＝引き継がない」
- ワークフローYAMLを `yaml.safe_load` でパース（13ステップ）。**最初に書いた埋め込みPythonは
  ブロックスカラーのインデントを壊して落ちた** —— YAMLの `run: |` 内に列0から始まる行は置けず、
  かといって全行を字下げすると Python 側が壊れる。shell だけで完結する形に書き直した
- `autopilot-drill --check`（21シナリオ・16コード全被覆）／`property-tests --check`／
  `check-escalation` / `check-emergency-stop` / `check-degradation` / `check-authority` /
  `autopilot-runs` / `autopilot-selfheal` / `autopilot-budget` / `check-routine-runs` の各 --check
- **検証していないこと**: 引き継ぎが本番で実際に起きるところ。死んだ占有が出る日を待つ以外に
  確かめる手が無い。ドリルが証明するのは判定の論理だけで、本物の切替演習は引き続き未実施

### データ鮮度・依頼の棚卸し

- BQ: このコンテナは資格情報を持たず `bq-preflight.mjs` が落ちる（BigQuery MCP も主系Actions
  環境には無い）。本日09:25 JSTの `seo-daily.yml` run 33454679748 の Export preflight ログの
  実測で **2026-08-10..2026-08-29・20/28日・28日窓は 2026-09-06 から**。レーンA/Bはそれまで
  正当化できない
- 資格情報2件を最新runで実測（§1-3）: `CLAUDE_CODE_OAUTH_TOKEN` は**本セッションが Checkout
  以降を実行していること自体が充足の証拠**、`GCP_SERVICE_ACCOUNT_JSON` は run 33454679748 の
  `Credentials are configured` が success。どちらも依頼から外した
- **新規依頼**: 副系3経路がどれも稼働していない（ccr-0730 / ccr-0830 は「停止 2026-08-28・
  意図的」、ccr-0920 は failed）。**今日直した引き継ぎは「死んだ占有を後続が拾う」仕組みなので、
  拾う側が動いていないと効かない。**再開の可否はオーナー判断
- トレンドレーダー: 本日は実施していない（レーンFの修理に充てた）
- ROUTINE_MCP_PROBE: unavailable 主系Actions環境にCCRのMCPが無い

### 台帳

- `data/autopilot-runs.json`: `ap-20260901-actions`（route: actions・outcome: shipped・
  lane: F・action: maintenance・`repair_of: ap-20260829-ccr0920`）と、
  `ap-20260901-owner-session`（PR #738 の修理を記録・`repair_of` に usage_limit 2件・
  `repair_note` に「及ぶ範囲は応答経路まで」）を追記
- `data/autopilot-status.json` を当日の内容で上書き。cost は月替わりで 2026-09 の集計に
  切り替えた（**runs 0 は「今月まだ計上が無い」であって「無料で動いた」ではない**）
- 費用: 当日runの実費は翌日09:00 JSTのアクチュエータ（append-cost）が入れる

### 次回への申し送り

**次に `takeover=true` の warning が出た日は、その run が本当に出荷まで行ったかを台帳で
確かめること。**拾ったのに出荷しなければ、穴は1段深いところにある。

猶予120分は主系の `timeout-minutes: 90` から置いた値で、**副系CCRの最長実行時間は測っていない。**
副系が120分を超えて走る回があるなら、そのとき奪われる側になる。

`property-tests.mjs` の乱択は `claimHasWork` / `claimAgeMinutes` をまだ振っていない
（レーンFの `may_modify` 外なので今回は触っていない）。組み合わせの網を新しい判定にも掛けるのが次の一手。

coverage-queue の pending は残り26件。故障が片付いた日はレーンEへ戻る。
