...
sitemap dry-run は通過。動的 viewport は実行環境に Playwright ブラウザエンジンがなく未計測で、
docs-only 変更について未観測を成功扱いしていない。BigQuery は認証がなく取得不能、
Codex 実費も観測経路が無いため、どちらもゼロではなく unknown。
