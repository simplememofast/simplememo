...
## 2026-08-10 追記: この章の前提が2つ外れた

### 誤り①「hero がわずか44本 ＝ 大半のページは読み切った人にしかCTAを見せていない」

方向は合っていたが、**根拠が足りていなかった**。hero の本数だけを見て読了率を見ていない。
GA4実測で **90%到達は 139/1,334 = 10.4%**。一方、検索クリックを持つ53ページのうち
39ページ（検索クリックの70%）が下部とnavにしかCTAを持たない。

対処は hero の追加ではなく、MEDIUM層に元々許されていた
**「Inline CTA 1箇所・文脈に沿った1文＋リンク」**の投入（9ページ・2026-08-10）。
その枠が空いていることに気づいていなかった。

### 誤り②「現状は全ページほぼ同一コピー」

**事実ではない。** cta-box 199個のうち **192種がユニーク**で、既に意図別に書き分けられている。
同一だったのは*リンクの本数*であってコピーではなかった。
したがって「意図別への置換」は施策として成立しない（既に済んでいる）。

### 見落としていた最大の構造穴: PCが行き止まり

```
app_store_click  60件 / 56ユーザー
  desktop        21件 / 18ユーザー（35%）  ← 押してもインストールできない
  mobile+tablet  39件 / 38ユーザー（65%）
```

PCでバッジを押すと開くのはWeb版App Storeで、そこからiPhoneには入らない。
QRは240ページ中2ページ（`/ai-tags/`）にしか無かった。**パターンは存在していて、
横展開されていなかっただけ。** 9ページに追加した（2026-08-10）。

天井は**月21クリック**。効果量ではなく「この21人は現に壁に当たっている＝判断に
統計的検出力が要らない」ことが実施理由である。詳細は
...
| **デバイス別の遷移** | ✅ GA4（イベント名 × デバイスカテゴリ）で取得可 |
| **QRのスキャン** | ❌ **不可**。押していないので `app_store_click` が発火しない |
| インストール数への接続 | ⚠️ App Store Connect側の手動突合が必要 |
| Activation / Paid への接続 | ❌ **未接続**。プロダクト分析基盤が無い |

**QRの行が示すとおり、今回のPC施策は現状の計測体制では効果を確認できない。**
`ct=*__qr` の集計は App Store Connect 側にしか出ず、それはオーナー作業である。
