...
**現在の運用（2026-09-15）:** 以下は旧計画の数値と不足条件。既存SEO Dailyからクエリ×ページを取得済みで、[今回の照合では4LPを維持](QUERY_PAGE_REVIEW_2026-09-15.md)と判断した。原収集の下限・匿名化は残り、取得できたことだけで統合しない。手動エクスポートは常時の前提ではない。
