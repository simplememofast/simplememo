...
PR594の最新照合headは `44f2d783a6063c1cce43ca43b6a6854a974dca9b`。main `57fcafe` の同時受付・Watch ACK保持・対話メモ修正まで取り込み、元の計測境界と宛先を保持した。明示選択の保存・候補参照、受付世代の引継ぎ、取消した予約の下書き復元、遅延した音声/画面応答の分離を追加した。最終native結果は `queue.json` の同headの証拠を参照する。

現headは関連10 suite・494 native testsが成功し、失敗/skipは0。未登録adapterの22件と実EKReminderオブジェクトの日付/通知設定を含む。実端末への保存・Siri実行・NSM到達の証拠ではない。
