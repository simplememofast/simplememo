...
### 5.2 GA4 UI操作（要手動）
1. 管理 → イベント → カスタムイベント `app_store_click` が計上され始めたら、
2. 「キーイベントとしてマーク」をON。
3. 探索レポートで「ページ別 app_store_click 率」を作成 → どのページがDLに効くかを可視化。
...
本フェーズの追加アクション:
- 各ページ冒頭に**自己完結の定義段落**（数値込み: 起動0.3秒・端末内AES-GCM・料金）。
...
### P0（今週・低工数・即効）
- [x] 高表示4ページのタイトル/件数矛盾の修正（commit 4f6d7e56）
- [ ] **GA4 `app_store_click` イベント実装 + キーイベント化**（§5）← 最優先
- [ ] オーファン3件へ内部リンク追加

### P1（4週間・最大レバー）
- [ ] **`lang.js` タイトル上書き修正**（§3.1、全150ページに波及／要レビュー）
- [ ] **英語EN実体化 Wave 1**（§4.3：note-to-email / captio-alternative / best-memo / send-email）
- [ ] best-memo クロスリンク26件の `10選→20選`（§3.2）
- [ ] カニバリ統合：比較/ランキング系を `best-memo-apps-2026` ハブに集約（§2.4）

### P2（四半期）
- [ ] EN実体化 Wave 2–3（vs/主要 + blog/glossary）
- [ ] 二重DOM全廃（SSG/ビルド導入）
- [ ] `AggregateRating` 展開・薄い多言語スタブの判断・リンカブルアセットで被リンク獲得
