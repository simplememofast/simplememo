...
2026-09-12追記：台帳上の本番観測済みは **4/11件**。Act経路の停止・復元を本番で追加確認（[実行記録](production-act-stop-exercise-20260912.md)）。
2026-09-09時点の記録：既存のCI失敗時に自動マージjobがスキップされ、修正後の検証成功でマージされた履歴を
確認した。[観測記録と検証の限界](auto-merge-stop-observation-2026-09-09.md)を参照。
9月9日の観測は既存履歴の確認で、当時は追加の本番停止や業務200全体の完了ではなかった。
...
## 2. 2026-08-26時点で本番未確認だったものと理由

以下は当時の記録。`auto-merge-verified-sha` は上記の2026-09-09観測でproductionへ更新した。
