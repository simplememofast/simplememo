...
| **主系の実績（改訂1で実測）** | **⚠ 主系GitHub Actionsは11回のうち一度も記事を出荷していない**（下記） |
...
### 4-1-2. 【改訂1・重要】主系は一度も出荷していない — 動いているのは副系

GitHub Actions API で `obsidian-autopilot.yml` の全11runを実測した結果：
...
**つまり、この12日間に出荷された記事・修正はすべて副系CCR Routineによるもの。**
主系はまだ一度も本番へ何も出していない。

**これは弱点ではなく、書き方を変えるべき事実。**
主系が11回連続で成果物を出せない間、**副系が毎日出荷を維持していた**。
冗長化が実際に効いた実例であり、「1本の自動化が止まったら全部止まる」という
記者の当然の疑問への回答になる。原稿では「主系・副系の二重化で維持している」と書く。
