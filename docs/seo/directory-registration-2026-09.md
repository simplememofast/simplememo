...
### 美崎栄一郎公式サイト（JA-016）—— プレミアム1か月無料のオファーコードを送付
...
- **12:23** `Simple Memo <support@simplememofast.com>` から返信で送付。本文の中身は、コード、使い方（アプリのURL → 引き換えURL → 反映されないときは「購入を復元」）、
  「無料期間が終わると自動で終了し、請求は発生しない」、「紹介の有無・内容は任せる」。送信前に差出人・宛先1件・件名・本文・署名を読み戻し、送信済みフォルダでも確かめた。
- コードはオーナーが App Store Connect で作った（こちらには ASC にサインインする手段が無い）。設定は月額プラン・無料・1か月・**自動更新しない**・カスタムコード・上限500・期限 2026-12-31。
  「自動更新しない」はオーナーの回答に基づいていて、こちらは画面を見ていない。**コードの文字列はこの公開リポジトリに書かない。**
...
| --- | --- | --- |
| できること | アプリの無料ダウンロード | プレミアムを無料・割引で提供（今回は1か月無料） |
...
- 次に頼まれたときの手順（男子ハック JA-007 も、窓口の案内で「プロモーションコード」に触れている）：
  ...
  カスタムコード（媒体ごとに別のコードにし、上限は小さく）。メールに「請求は発生しない」と書くのは、**自動更新しない設定を確かめてから**。
...
| 12 | 美崎様向けオファーコードの上限（500） | ASC の操作はサインインが要る | 記事などで共有されると、最大500人が1か月無料になる。意図していなければ、引き換えを確認したあと ASC の「無効化」で新しい引き換えを止められる（引き換え済みの分への影響は未確認）。止めるかどうかはオーナーが判断する（§5.14） |
