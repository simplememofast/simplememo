...
- 送信完了：**4 件**（bamka.info / 技術評論社 gihyo.jp / ガジェットショット / AAPL Ch.）
- 判断で送らなかった：**2 件**（気になる、記になる…＝公開コメント欄のため／Macお宝鑑定団＝中断）
...
- **Macお宝鑑定団（JA-002）** — 入力の途中で中断（下記の identity 修正のため）。1文字も送信していない。

### identity policy 違反 —— 送信済み4件に個人名が入っている
...
- **送信済み4件に開発者個人の実名が入っている（identity policy 違反）。** 訂正連絡を出すかどうかの判断が要る。
