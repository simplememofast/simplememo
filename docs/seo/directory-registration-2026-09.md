...
- **`data/site-constants.json` がストアとずれている。** 2026-09-19 の iTunes Lookup では
  version 5.8.66 / 評価 4.12・26件だが、`site-constants.json` は 5.8.9 / 4.2・25件。
  評価は JSON-LD に出ているので、放置すると構造化データが実態と食い違う。
  外部に出す文面ではこの2つを使わないことで回避したが、`scripts/sync_constants.js --write` での
  同期は別途必要。
