...
- ~~**Mac Fan に送るなら電話番号を公開するか決める必要がある。**~~
  **2026-09-22 にメール窓口 `mnp-macfan-release@mynavi.jp` へ送信済み（§5.9）。**
  ただし**受付フォーム自体は電話番号必須のままで未解決**。フォーム経由でも出したいなら、
  会社の電話番号を公開するかどうかを決める必要がある（番号の創作はしない）。
