...
| 42 | 公開ページの版と評価の件数（5.8.66・26件。App Store の実物は 9/26 に 5.9.9・27件）を更新するか（§5.32） | Codex の #1576（ストア検査の修正、draft）が「公開ページの訂正と、元のリリースの門（original release gates）が整うまで draft」としている。約40ファイルに及び、門の中身はこちらで確かめられない | 更新するなら #1576 を先に入れ、`sync_constants.js --write` でそろえる PR を作る |
