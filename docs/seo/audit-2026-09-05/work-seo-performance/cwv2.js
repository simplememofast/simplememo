...
(async () => {
  ...
  for (const p of PAGES) {
    ...
    let rec = { page: p, tag: TAG, navStatus, wall: Date.now() - t0, ...m, requests: list.length, totalBytes, byType, statuses, thirdParty, fonts, failed, console: consoleMsgs, docHeaders: (list.find(r => r.url === BASE + p || r.url === BASE + p + '/') || {}) };
    delete rec.docHeaders.timing;

    const extra = await page.evaluate(() => {
      const m = window.__m || {};
      const fontsRT = performance.getEntriesByType('resource').filter(e => /\.woff2/.test(e.name)).map(e => ({ f: e.name.split('/').pop().split('?')[0], start: Math.round(e.startTime), end: Math.round(e.responseEnd), bytes: e.transferSize }));
      const cv = [...document.querySelectorAll('*')].filter(el => getComputedStyle(el).contentVisibility === 'auto').map(el => ({ sel: el.tagName + '.' + String(el.className).trim().split(/\s+/)[0], cis: getComputedStyle(el).containIntrinsicSize, h: Math.round(el.getBoundingClientRect().height), top: Math.round(el.getBoundingClientRect().top + scrollY) }));
      const imgs = [...document.images].filter(i => i.getBoundingClientRect().width > 0).map(i => ({ src: (i.currentSrc || '').replace(location.origin, ''), attr: i.getAttribute('width') + 'x' + i.getAttribute('height'), nat: i.naturalWidth + 'x' + i.naturalHeight, box: Math.round(i.getBoundingClientRect().width) + 'x' + Math.round(i.getBoundingClientRect().height), top: Math.round(i.getBoundingClientRect().top + scrollY), loading: i.getAttribute('loading') }));
      const fontsLoaded = [...document.fonts].map(f => f.family + ' ' + f.weight + ' ' + f.status + ' ' + (f.unicodeRange || '').slice(0, 20));
      return { fontsRT, cv, imgs, fontsLoaded, longtasksDetail: (m.longtasks || []).map(l => ({ t: Math.round(l.t), d: Math.round(l.d) })) };
    });
