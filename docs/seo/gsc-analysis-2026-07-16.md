...
検証結果: 両スラッグとも `git log --all -S` でコミット0件・現行ソース0件・sitemap非掲載。既知の捏造パターン(HN風英語スラッグ+自社テーマ模倣: outbox/inbox-task管理は当サイトの実コンテンツ用語)と同型で、**バックリンク・インジェクション攻撃の継続**を示す。対応は確立済みポリシーどおり: **404維持・リダイレクト/410不要**。リンク元調査・disavow追跡も**07-02オーナー決定によりスキップ**(Googleは捏造スパムリンクをアルゴリズムで無視するため。GSCに手動による対策が表示された場合のみ再浮上)。
...
2. インデックス登録バケットの「検証を開始」ボタンは**リダイレクト/404バケットでは押す必要なし**(仕様上の表示でエラーではない)。クロール済み未登録も放置で可

(捏造404のリンク元調査・disavow追跡は07-02オーナー決定によりオーナーアクションから除外 — §2-1参照)
