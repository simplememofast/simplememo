...
| 172 | `claude/i18n-phase3-analytics-cdn` | GA4 page_language（197 ファイル）+ IndexNow 鍵 + CDN ヘッダー | `scripts/inject_ga4_page_language.py`, `scripts/notify_indexnow.py`, `3b4a3c278a4cc17ab7e03d6e7739bd21.txt`, `_headers` |
...
- IndexNow キーが本番に配信済（**注意：このプロジェクトには現在 2 つのキーが存在。§2.1 参照**）

#### 既存・新規キーの状況

| キー | 配信ファイル | 由来 |
|---|---|---|
| `dda35fda390ffabbcce681394b3a57cc` | `/dda35fda390ffabbcce681394b3a57cc.txt` | **既存**: `.indexnow-key` ファイル + `scripts/indexnow-notify.js` (Node) |
| `3b4a3c278a4cc17ab7e03d6e7739bd21` | `/3b4a3c278a4cc17ab7e03d6e7739bd21.txt` | **新規**: i18n Phase 3 PR #172 で追加 + `scripts/notify_indexnow.py` (Python) |

両方とも本番で 200 を返す状態。Bing には **どちらか一つ**を登録する。**§2.1 で重複を解消した後**、残った方を登録するのが clean。

#### 手順（§2.1 完了後を想定）
1. Bing Webmaster Tools にサインイン
2. 左メニュー「設定」→「API アクセス」（または「IndexNow」）
3. 「API キーを生成または登録」
4. キーを入力（§2.1 の決定により `dda35fda...` または `3b4a3c27...` のどちらか）
5. キー検証 URL: `https://simplememofast.com/<KEY>.txt` を確認
6. 登録完了
...
```bash
# Python 版（i18n Phase 3 で追加）
python3 scripts/notify_indexnow.py --dry-run   # 内容確認
python3 scripts/notify_indexnow.py             # 実行
...
- [ ] `notify_indexnow.py`（または `indexnow-notify.js`）を実行して 200/202 レスポンスが取得できる
...
#### 新規インフラ（撤去すべき）
- `/3b4a3c278a4cc17ab7e03d6e7739bd21.txt`
- `/scripts/notify_indexnow.py`

#### 推奨アクション
**選択肢 A（推奨）**: 新規鍵を撤去、既存インフラ（Node 版）に統一
1. `claude/i18n-cleanup-indexnow-dup` ブランチ作成
2. 削除：
   - `/3b4a3c278a4cc17ab7e03d6e7739bd21.txt`
   - `/scripts/notify_indexnow.py`
3. PR コメントに「既存の `indexnow-notify.js` に統一。詳細は `docs/indexnow-setup.md` を参照」と記載
4. PR を auto-merge

**選択肢 B**: 新規鍵に統一（既存撤去）
- 既存 Node 版が CI に組み込まれているため、撤去すると CI が壊れる。**非推奨**

#### 完了条件
- [ ] 本番に IndexNow 鍵ファイルが 1 つだけ存在
- [ ] Bing IndexNow に登録した鍵と一致
- [ ] CI 動作（`seo-check.yml`）に影響なし
...
### 3.3 `/en/captio-alternative/` MTPE 拡張（Phase 4-c）
...
### 5.2 IndexNow 鍵（重複中、§2.1 で解消予定）

```
EXISTING (keep)       = dda35fda390ffabbcce681394b3a57cc
                        - file: /dda35fda390ffabbcce681394b3a57cc.txt
                        - script: scripts/indexnow-notify.js
                        - CI: .github/workflows/seo-check.yml で自動呼び出し
NEW (remove)          = 3b4a3c278a4cc17ab7e03d6e7739bd21
                        - file: /3b4a3c278a4cc17ab7e03d6e7739bd21.txt
                        - script: scripts/notify_indexnow.py
```
...
- [ ] **Phase 4-c**: `/en/captio-alternative/` MTPE 拡張（§3.3 残作業）
...
# 5. IndexNow 通知
# 既存（推奨）:
node scripts/indexnow-notify.js          # 過去 1 日の更新を通知
node scripts/indexnow-notify.js --all    # 全 URL
# 新規（重複、撤去予定）:
