...
- 副コピー: 「Captio式シンプルメモ — 起動 約1秒・ワンタップ送信」
- フォント: Noto Sans JP
...
3. **MTPE 完了判定**：レビューレポート **§9** の 4 階層基準を全クリアしたら統合可
   - Layer 1: signoff にネイティブ宣言
   - Layer 2: 3 ファイル存在、`final.md` と `draft.md` に diff あり、changelog ≥5 件
   - Layer 3: §3.2〜§3.6 チェックリスト全項目回答済
   - Layer 4: 公開 7 日以内に Search Console 警告ゼロ（公開後判定）
4. レビューレポート §3.4 / §3.5 のリンク挿入が完了している
5. レビューレポート §3.3 のフィーチャー比較表が実測値ベース or Yes/No 簡素化済
6. レビューレポート §5 のチェックリストに従い `/en/captio-alternative/index.html` の `<main>` 内本文を `final.md` で置換
7. `python3 scripts/inject_faq_schema.py` を再実行（FAQ が 9 → 12 問になるため JSON-LD を再生成）
8. CSS バージョン bump（`?v=YYYYMMDD`）
9. PR → 自動マージ → デプロイ
10. レビューレポート §6 の検証チェックリストに従い 24h / 7日 / 30日後の各検証を実施
