...
## 2026-09-06: 2回目以降の実行がGit設定で停止する問題

05:39 UTCの実サービス確認で、launchdはruns=22・last exit code=1、
runner.logの直近2行は `Command failed: git exit 5` だった。
ログに失敗時刻が含まれないため、障害の開始時刻は確定できない。
直前に確認済みの全処理成功は02:13 UTCの9回目で、
その後にはGitHubの処理を確認して待機した回もある。

専用cloneのcredential helperには、空値による継承解除と既存ghプログラムの
2エントリがあった。linked worktreeはこの設定を共有するため、
次回の単一値 `git config ... ''` が多値の上書きを拒否してexit 5となる。
実Gitの隔離検体でも1回目は成功し、2回目の同じ設定でexit 5を再現した。

設定の初期化を `--replace-all` に変更し、続けて既存のgh helperを追加する。
使う認証・権限・15分間隔・待機判定・緊急停止・成功証跡の条件は同じ。
8本目のPython自己検査ではローカルの実Git remoteから3回続けてclone/fetch/
worktree/config/cleanupの処理を行い、helperが毎回同じ2エントリであることを確認する。
NodeとGitHubの受付判定だけを検体に置き換え、手動probeは成功証跡を残さない。
全8検査が成功し、`--replace-all`を外した変異は2回目の実Git exit 5で失敗した。

これは自然に発生した監視サービス障害の診断と手動修正であり、
検体成功を本番の自動復旧・成果決済の実績に加えない。
検証済みmainから導入した後、自然なintervalでの全処理成功を別途確認する。

## 2026-09-20: 配信確認が既定の30件から落ちる問題

PR #1470 の実際のmerge SHAには42件のcheck runがあり、Cloudflare Pagesの
成功記録は既定の先頭30件に含まれていなかった。宣言・実装順・SEO検証・
本番の宣言JSONは確認できたが、monitorは配信未確認のまま評価を進められなかった。

配信・SEO確認はcheck名で絞り込み、100件ずつ最大5ページを読む。
総件数・各ページ件数・重複ID・名前・対象SHAを確認し、不完全な一覧や
途中の件数変化、上限超過は停止する。配信は引き続き最大IDの記録を使うため、
新しい失敗を古い成功で置き換えない。既存の所有者と起動間隔を維持する。

この修正の読み取り検証では元の配信時刻が2026-09-19 02:35:22 UTCと確認できた。
元の1日評価窓は9月20日のJST全日で、9月21日0時より前はpendingのまま。
宣言・台帳・評価期間・算式は変更せず、元のmonitorによる期間満了後の実測決済を待つ。
