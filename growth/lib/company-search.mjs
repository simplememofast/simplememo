...
function selectionScope(page, pages, record=null, {now=new Date(),followup=false}={}) {
  try {
    ...
    const scope = experimentScope({page: typeof page === 'string' ? page : explicit?.[0] ?? '', pages: explicit,change_paths:record?.change_paths,supporting_changes:record?.supporting_changes});
