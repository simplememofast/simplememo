...
非公開の分析結果を使う場合は `evaluate` に `--private-evidence-dir ~/.config/simplememo/company-os/experiment-evidence` を付ける。既存の資料検査を通してから、正規化された証拠詳細を所有者の0700ディレクトリへ0600ファイルとして保存し、SHA-256を再照合する。元のレビューmanifest・API成果物は複写せず、既存のprivate storeに保持する。正準台帳には判定と内容ハッシュ付き参照だけを残す。Git内の保存先は拒否する。公開される `--note` には数値・秘密・私有パスを書かない。参照先は `experiment-evidence-<sha256>.json`。次回評価・監査時はこのファイルのハッシュと実験ID・判定を照合して読む。私有成果物が無ければ「証拠を読めない」であり、成功の追認に使わない。保存方式は証拠の受け入れ条件や因果の限界を変えない。
