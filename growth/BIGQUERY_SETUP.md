...
異なるURLが同じサイト内パスになる場合、BigQuery取得は正規化後に
`pages` と `query-pages` を合算する。CSV取得に既にある `mergeByKey` を
共用し、クリック数・表示回数をすべて保持する。順位は表示回数で重み付けし、
BigQueryの精度は丸めない。CSVの従来の小数2桁丸めは維持する。
`query-pages` はクエリとページの組をキーとし、異なるクエリを混ぜない。

合算数は新しい出力の `meta.bigquery.canonical_page_normalization` に記録する。
過去のスナップショット、実験の基準値、取得窓、匿名クエリの扱い、
最小表示回数、サイト全体の分母は変更しない。欠けたキーや未処理の重複は
引き続き失敗とし、暗号化受け渡しの品質チェックを緩めない。
修復確認は取得元・実行IDを保持し、手動の再実行と自然定期実行を区別する。
取得・暗号化・取り込みの成功を確認するまで復旧済みにしない。
